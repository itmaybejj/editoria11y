	...
	/* references overridden */
		if (Constants.Global.headless === false) {
			if (pageText.length === 0) {
				UI.readabilityInfo.innerHTML = Lang._('READABILITY_NO_CONTENT');
			} else if (readabilityResults.wordCount > 30) {
				UI.readabilityInfo.innerHTML = `${Math.ceil(readabilityResults.score)} <span class="readability-score">${readabilityResults.difficultyLevel}</span>`;
				UI.readabilityDetails.innerHTML = `<li><strong>${Lang._('AVG_SENTENCE')}</strong> ${Math.ceil(readabilityResults.averageWordsPerSentence)}</li><li><strong>${Lang._('COMPLEX_WORDS')}</strong> ${readabilityResults.complexWords}%</li><li><strong>${Lang._('TOTAL_WORDS')}</strong> ${readabilityResults.wordCount}</li>`;
			} else {
				UI.readabilityInfo.textContent = Lang._('READABILITY_NOT_ENOUGH');
			}
		}
	// } override
	return results;
