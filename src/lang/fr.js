...
export const testNames = {
	...
	LINK_IDENTICAL_NAME: 'Ce lien décrit‑il de manière unique sa destination ?',
	...
	LINK_STOPWORD: 'Ce lien décrit‑il correctement sa destination ?',
...
<li>Changement <a href="https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Roles/button_role#associated_aria_roles_states_and_properties">d’attributs d’état</a> :<br>"Lecture/Lecture, activé", "Détails, réduit/Détails, développé."</li>
</ul>
<p>Il suffit de ne pas changer ces deux choses en même temps. Passer de “Lecture” à “Pause, activé” indiquerait que le lecteur est en pause, non en lecture !</p></div>`,
...
<ul><li>En‑tête niveau 1 : titres de page
<ul><li>En‑tête niveau 2 : sujets principaux
<ul><li>En‑tête niveau 3 : sous‑sujets</li></ul></li></ul></li></ul></div>`,
...
<ul><li>Ils jouaient sous la pluie.</li>
	<li>Les nouveaux uniformes ont un motif de dragon.</li>
	<li>Elle a marqué le but de la victoire depuis le côté gauche !</li></ul></div>`,

    links: `<div class="why"><p>Les utilisateurs parcourent les pages par les liens et utilisent la recherche interne pour les trouver par nom. Des liens efficaces sont donc significatifs, uniques et concis :</p>
<ul>
...
<li>Pas concis : "<a href="https://webaim.org/techniques/hypertext/link_text">Cliquez ici pour en apprendre davantage sur les liens significatifs</a>"</li>
    ...
    imageLinks: `<div class="why"><p>Le but du texte alternatif est de fournir une alternative au sens d’une image. Pour une image liée, le sens correspond à la destination du lien :
	<ul>
...
export const tips = {
	ALT_FILE_EXT: `<p>Les lecteurs d’écran liront cette URL, souvent lettre par lettre. Cela ne transmet probablement pas la même signification que de voir l’image.</p><p>${why.fix}Ajoutez un texte alternatif vide (alt="") si cette image est purement décorative et doit être ignorée par les lecteurs d’écran, ou ajoutez un texte alternatif descriptif.</p>${why.images}`,

	ALT_MAYBE_BAD: `<p>Description fournie pour cette image : <strong>"%(alt)"</strong></p><p>${why.fix}Définissez le texte alternatif de cette image sur une description concise de ce que l’image signifie dans ce contexte.</p>${why.images}`,

	ALT_PLACEHOLDER: `<p>Description fournie pour cette image : <strong>"%(alt)"</strong></p><p>${why.fix}Définissez le texte alternatif de cette image sur une description concise de ce que l’image signifie dans ce contexte.</p>${why.images}`,
	...
	BTN_EMPTY: `<p>${why.fix}Utilisez toute méthode valide pour indiquer aux lecteurs d’écran l’action de ce bouton : texte, texte alternatif sur une icône, ou attribut title.</p>`,
	...
	CONTRAST_WARNING: 'Une image de fond ou un dégradé empêche ce vérificateur de connaître précisément la couleur derrière ce texte. Utilisez l’outil de sélection de couleur ci‑dessous pour vérifier manuellement.',

	DUPLICATE_ID: `<p>Les IDs sont utilisés sur cette page pour des étiquettes ou des cibles de liens, ce qui signifie qu’ils doivent être uniques.</p><p>${why.fix}Modifiez cet ID : <strong>#%(id)</strong></p><div class="why"><p>Dans la plupart des systèmes de gestion de contenu, cela provient d’un champ “name” ou “id” dans les propriétés de l’élément. En HTML, il s’agit d’un attribut : <code><a id="MY-ID"></code></p></div>`,

	DUPLICATE_TITLE: `<p>${why.fix}Supprimez l’attribut <code>title</code> du lien.</p><div class="why"><p>Note : les info‑bulles <code>title</code> n’apparaissent qu’au survol de la souris. Elles ne sont pas visibles sur mobile ou via le clavier ; de nombreux utilisateurs ne les verront jamais. Elles ne doivent jamais contenir d’information importante ou unique.</p></div>`,

	EMBED_AUDIO: `<p>Si l’audio contient de la parole, une <a href="https://www.w3.org/WAI/media/av/transcribing/">alternative textuelle</a> doit être fournie sur cette page ou via un lien.</p><p>Note : une personne doit relire et corriger les transcriptions générées automatiquement, et identifier les locuteurs et sons significatifs.</p>`,

	EMBED_DATA_VIZ: `<p>Les visualisations intégrées sont souvent difficiles, voire impossibles, à utiliser pour les technologies d’assistance. Elles peuvent être difficiles à comprendre pour les personnes malvoyantes ou daltoniennes, et nécessiter un défilement horizontal important sur mobile.</p><p>${why.fix}À moins que cette visualisation ne présente un fort contraste visuel, soit entièrement utilisable au clavier <strong><em>et</em></strong> décrite par un lecteur d’écran, ajoutez un format équivalent (description textuelle, tableau de données, feuille de calcul téléchargeable), puis ignorez cette alerte.</p>`,

	EMBED_GENERAL: 'Les vérificateurs automatiques ne peuvent pas analyser le contenu à l’intérieur des éléments intégrés. Assurez-vous que toutes les images ont un texte alternatif, que les vidéos ont des sous‑titres, que le texte a un contraste suffisant, et que les liens et boutons sont <a href="https://webaim.org/techniques/keyboard/">accessibles au clavier</a>, puis ignorez cette alerte.',
	...
	HEADING_LONG: `<p>${why.fix}À moins qu’il ne s’agisse d’un titre fixe (comme celui d’un article publié), raccourcissez‑le pour aider les personnes à parcourir le contenu.</p>${why.headings}`,
	...
	IMAGE_ALT_TOO_LONG: `<p>Les textes alternatifs sont lus comme une phrase continue par les lecteurs d’écran ; s’un élément est manqué, il faut tout réécouter.</p><p>Le texte alternatif de %(altLength) caractères pour cette image est : <em class="ed11y-small">%(ALT_TEXT)</em></p><div class="why"><p>Astuce : les images complexes nécessitant plus qu’une phrase doivent généralement avoir une légende <strong>visible</strong> ou une alternative détaillant les points importants. Il est acceptable d’y renvoyer :</p><ul><li>"Affiche pour la danse de vendredi ; détails dans la légende."</li><li>"Graphique montrant une baisse de 10 % ; détails dans le tableau."</li></ul></div>`,

	IMAGE_DECORATIVE: `<p>Cette image a été masquée pour les lecteurs d’écran via un texte alternatif vide. Seules les images sans signification (icônes redondantes, textures décoratives) doivent être masquées de cette manière.</p><p>${why.fix}Si cette image apporte une information, fournissez un texte alternatif.</p>${why.images}`,
	...
	LABELS_ARIA_LABEL_INPUT: '<p><strong {B}>Étiquette invisible :</strong> <strong {C}>%(TEXT)</strong></p><p>Vérifiez qu’il existe une étiquette visible, qu’elle reste visible lorsque du texte est saisi, et qu’elle correspond à l’étiquette invisible.</p><div class="why"><p>Des étiquettes visibles uniquement via title ou placeholder disparaissent quand on commence à écrire, ce qui complique la relecture et la saisie.</p></div>',

	LABELS_INPUT_RESET: `<p>Les boutons de réinitialisation sont facilement activés par erreur, provoquant une perte de données sans possibilité d’annuler.</p><p>${why.fix}À moins de réinitialiser un seul champ, envisagez de le retirer ou d’offrir une confirmation avant l’action.</p>`,

	LABELS_MISSING_IMAGE_INPUT: 'Le bouton image n’a pas de texte alternatif. Ajoutez un texte alternatif pour fournir un nom accessible comme <em>Chercher</em> ou <em>Envoyer</em>.',

	LABELS_MISSING_LABEL: 'Aucune étiquette n’est associée à ce champ. Ajoutez un <code>id</code> au champ et un attribut <code>for</code> correspondant à l’étiquette.',
	...
	LABEL_IN_NAME: `<p>Le texte visible de cet élément semble différent de son nom accessible. Cela peut dérouter les utilisateurs de lecteurs d’écran et gêner le contrôle vocal.</p><p>${why.check}Assurez-vous que l’étiquette visible commence par l’étiquette invisible et n’ajoute aucune information significative supplémentaire.</p><p><strong>Étiquette invisible :</strong> "%(TEXT)"</p>`,

	LINK_ALT_FILE_EXT: `<p>Le texte alternatif de cette image contient "%(alt)", ce qui indique probablement un nom de fichier plutôt qu’un nom significatif pour le lien.</p><p>${why.fix}Définissez le texte alternatif sur le nom de la destination du lien.</p><div class="why"> <p>Le texte alternatif doit refléter la signification, pas le contenu brut : pour une image liée, la signification est la destination du lien.</p><ul><li>"Page avec texte" décrit l’image, pas le lien.</li><li>"IMG_1234.jpg" est un simple nom de fichier.</li><li>"<strong><em>Formulaire d’inscription (doc)</em></strong>" est une vraie destination.</li></ul></p></div>`,

	LINK_ALT_MAYBE_BAD: `<p>Le texte alternatif de cette image est un placeholder : "<strong>%(alt)</strong>".</p><p>${why.fix}Définissez le texte alternatif sur la destination du lien.</p>${why.imageLinks}`,
	...
	LINK_IDENTICAL_NAME: `<p>Plusieurs liens menant à des destinations différentes utilisent le même libellé "<strong>%(TEXT)</strong>".</p><p>${why.fix}Réécrivez les liens afin que chaque libellé corresponde clairement à sa destination unique.</p>${why.links}`,
	...
	LINK_IMAGE_LONG_ALT: `<p><a href="https://webaim.org/techniques/hypertext/link_text#alt_link">Le texte alternatif d’une image liée sert à décrire la destination du lien</a>. Les liens doivent être brefs et clairs : les utilisateurs de lecteurs d’écran parcourent souvent uniquement la liste des liens. Un texte alternatif long indique souvent qu’il décrit l’image plutôt que la destination.</p>Le texte alternatif de %(altLength) caractères de cette image est : <em>%(ALT_TEXT)</em>`,
	...
	LINK_NEW_TAB: `<p>${why.fix}Configurez ce lien pour s’ouvrir dans le même onglet, ou <a href="https://itmaybejj.github.io/linkpurpose/">prévenez les utilisateurs</a> avant l’ouverture dans un nouvel onglet.</p><div class="why"><p>Les utilisateurs peuvent toujours choisir d’ouvrir un lien dans un nouvel onglet. Forcer l’ouverture peut être déroutant, surtout pour les technologies d’assistance.</p><p>Note : les liens dans les formulaires ouvrent souvent un nouvel onglet pour éviter la perte de données.</p></div>`,
	...
	LINK_URL: `<p>${why.fix}Changez ce lien pour utiliser le titre de sa destination ou sa fonction.</p><div class="why"><p>Les utilisateurs parcourent la page par ses liens — en particulier les utilisateurs de lecteurs d’écran.</p><p>Les URLs en texte de lien sont difficiles à parcourir et à rechercher.</p></div>`,

	META_LANG: `<p>${why.fix}Ajoutez un <a href="https://www.w3.org/International/questions/qa-html-language-declarations">attribut de langue sur la balise HTML</a>.</p><div class="why"><p>Astuce : les lecteurs d’écran prononcent les mots selon la langue indiquée. Une mauvaise langue produit un discours incompréhensible.</p></div>`,
	...
	QA_BLOCKQUOTE: `<p>Le format blockquote indique aux lecteurs d’écran qu’il s’agit d’une citation. Les citations courtes sont souvent en réalité des titres.</p><p>${why.fix}Si ceci est un en‑tête, utilisez un style d’en‑tête pour qu’il apparaisse dans la structure de la page.</p>${why.headings}`,
	...
	QA_JUSTIFY: `<p>Le texte justifié ajoute des espaces irréguliers entre les mots, ce qui rend la lecture plus difficile pour de nombreuses personnes.</p><p>${why.fix}Utilisez un alignement à gauche.</p>`,
	...
	QA_STRONG_ITALICS: `<p>${why.fix}Réservez le gras et l’italique aux mots ou phrases clés.</p><div class="why"><p>Note : si c’est une citation, utilisez la balise blockquote.</p></div>`,
