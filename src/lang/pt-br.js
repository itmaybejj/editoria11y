...
export const tips = {
	...
	ALT_FILE_EXT: `<p><span style="display: none">%(alt)</span>Texto alternativo: <i>%(ALT_TEXT)</i></p><p>Leitores de tela lerão este URL, muitas vezes caractere por caractere. Isso provavelmente não transmite o mesmo significado que ver a imagem.</p><p>${why.fix}Adicione alt vazio (alt="") se for uma decoração sem significado que deve ser ignorada por leitores de tela, ou adicione um texto alternativo descritivo.</p>${why.images}`,
	ALT_MAYBE_BAD: `<p>Texto alternativo: <i>%(alt)</i></p><p>${why.fix}Defina o texto alternativo como uma descrição concisa do que esta imagem significa neste contexto.</p>${why.images}`,
	ALT_MAYBE_BAD_WARNING: `<p>Texto alternativo: <i>%(alt)</i></p><p>${why.fix}Defina o texto alternativo como uma descrição concisa do que esta imagem significa neste contexto.</p>${why.images}`,
	ALT_PLACEHOLDER: `<p>Texto alternativo: <i>%(alt)</i></p><p>${why.fix}Defina o texto alternativo como uma descrição concisa do que esta imagem significa neste contexto.</p>${why.images}`,
	ALT_UNPRONOUNCEABLE: `<p>Texto alternativo: <i>%(alt)</i></p><p>Este texto alternativo contém apenas símbolos que não podem ser pronunciados e/ou espaços. Leitores de tela vão indicar que há uma imagem e, em seguida, fazer uma pausa estranha ou ler algo ininteligível.</p><p>${why.fix}Adicione um texto alternativo descritivo ou forneça um alt completamente vazio (alt="") se for apenas um ícone ou espaçador que deve ser ignorado.</p>${why.images}`,
	...
	EMBED_UNFOCUSABLE: `<p>Este atributo instrui teclados e dispositivos assistivos a pular o elemento. A menos que o conteúdo deste iframe não contenha links, botões ou campos de formulário e não possa ser rolado, este atributo precisa ser removido.</p>`,
	...
	HIDDEN_FOCUSABLE: `<p><strong>Elemento:</strong> <code>%(ELEMENT)</code></p><p>Este elemento tem <code>aria-hidden="true"</code> mas recebe foco pelo teclado. Para escondê-lo de leitores, adicione <code>tabindex="-1"</code>; caso contrário, remova <code>aria-hidden</code>.</p>`,
	...
	LABEL_IN_NAME: `<p><strong>Texto visível:</strong> <i>%(TEXT)</i></p><p><strong>Nome acessível:</strong> <i>%(TEXT)</i></p><p>O texto visível deste elemento parece ser diferente do nome acessível. Isso pode confundir usuários de leitores de tela e prejudicar o uso por comandos de voz.</p><p>${why.fix}Certifique-se de que o rótulo visível comece com o texto do rótulo invisível e não contenha informações relevantes que estejam ausentes do rótulo invisível.</p>`,
	LINK_ALT_FILE_EXT: `<p><span style="display: none;">%(ALT)</span>Texto alternativo: <i>%(alt)</i></p><p>Este texto alternativo provavelmente é um nome de arquivo, e não um rótulo significativo para o link.</p><p>${why.fix}Defina o texto alternativo desta imagem como o nome do destino do link.</p><div class="why"><p>O objetivo do texto alternativo é fornecer uma alternativa para o significado da imagem, não para o que ela contém. No caso de uma imagem que é um link, o significado é o destino do link:</p><ul><li>"Página com texto" descreve a imagem, não o link.</li><li>"IMG_1234.jpg" é apenas um nome de arquivo.</li><li>"Formulário de inscrição para o evento (.doc)" é o destino do link.</li></ul></p></div>`,
	LINK_ALT_MAYBE_BAD: `<p>Texto alternativo: <i>%(alt)</i></p><p>${why.fix}Defina o texto alternativo desta imagem como o nome do destino do link.</p>${why.imageLinks}`,
	LINK_ALT_MAYBE_BAD_WARNING: `<p>Texto alternativo: <i>%(alt)</i></p><p>${why.fix}Defina o texto alternativo desta imagem como o nome do destino do link.</p>${why.imageLinks}`,
	LINK_ALT_UNPRONOUNCEABLE: `<p>Alt <i>%(ALT_TEXT)</i> tem caracteres impronunciáveis — não descreve a ligação.</p><p>${why.fix}Use alt que descreva a função/destino do link.</p>${why.imageLinks}`,
	...
	LINK_IMAGE_ALT: `Verifique se o alt descreve o destino da ligação:</p><p> {L} {ALT} <i>%(ALT_TEXT)</i></p>${why.imageLinks}`,
	...
	LINK_MAYBE_BUTTON: `<p>Este link possui um destino inválido e contém a palavra <i>%(NAME)</i>. Isso sugere que talvez não seja realmente um link, e sim controle algum comportamento de script na página.</p><p>${why.fix}substitua o link por um <a href="https://www.w3.org/WAI/ARIA/apg/patterns/button/">botão acessível</a>, ou corrija o destino do link.</p><div class="why"><p>Dica: tecnologias assistivas tratam botões e links de forma diferente. Usar o elemento HTML correto garante que os usuários saibam quais atalhos de teclado usar e qual ação será acionada.</p></div>`,
	...
	LINK_PLACEHOLDER_ALT: `<p>Alt é um placeholder: <i>%(alt)</i>.</p><p>${why.fix}Defina o alt para descrever o destino.</p>${why.imageLinks}`,
	LINK_STOPWORD: `<p>Texto do link: <i>%(text)</i></p><p>${why.fix}Use um texto de link que descreva o destino.</p>${why.links}`,
	LINK_STOPWORD_ARIA: `<p>Há nome acessível via ARIA, mas o texto visível é genérico: <i>%(ERROR)</i>.</p><p>${why.fix}Forneça texto visível significativo e consistente com o nome acessível.</p>${why.links}`,
	LINK_SUS_ALT: `<p>O texto alternativo desta imagem contém a palavra <i>%(alt)</i>, o que geralmente indica que ele não está descrevendo o destino do link.</p><p>Texto alternativo: <i>%(ALT_TEXT)</i></p><p>${why.fix}Garanta que o texto alternativo descreva o destino ou propósito do link.</p>${why.imageLinks}`,
	...
	QA_IN_PAGE_LINK: `<p><strong>ID:</strong> <code>%(ID)</code></p><p>O destino desta âncora interna não existe na página.</p><div class="why"><p>Para desenvolvedores: se depende de JavaScript, valide a operação por teclado antes de ignorar.</p></div>`,
