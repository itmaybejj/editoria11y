...
export const tips = {
	...
	BTN_TIP: `${why.buttons}`,
	BTN_ROLE_IN_NAME: `<p><strong>Rótulo para leitores de tela:</strong> <i>%(TEXT)</i></p><p>Leitores de tela usam a palavra "button" para anunciar que estão descrevendo um botão, portanto essa palavra é redundante.</p><p>${why.fix}O rótulo do botão deve corresponder à sua ação. Se o rótulo visível for um ícone em vez de texto, nomeie o botão com o significado do ícone, por exemplo, "Reproduzir", "Buscar" ou "Menu".</p>`,
	...
	LABEL_IN_NAME: `<p><strong>Texto visível:</strong> <i>%(TEXT)</i></p><p><strong>Rótulo para leitores de tela:</strong> <i>%(TEXT)</i></p><p>O texto visível deste elemento parece ser diferente do nome acessível. Isso pode confundir usuários de leitores de tela e prejudicar o uso por comandos de voz.</p><p>${why.fix}Certifique-se de que o rótulo visível comece com o texto do rótulo invisível e não contenha informações relevantes que estejam ausentes do rótulo invisível.</p>`,
	...
	LINK_IMAGE_ALT: `<p><strong>Texto alternativo:</strong> <i>%(ALT_TEXT)</i></p><p>${why.fix}Garanta que este texto alternativo descreva o destino do link, não apenas o conteúdo visual da imagem:</p>${why.imageLinks}`,
	...
	QA_IN_PAGE_LINK: `<p><strong>Link:</strong> <i>%(TEXT)</i></p><p><strong>URL:</strong> <code>#%(ID)</code></p><p>O destino desta âncora interna não existe na página.</p><div class="why"><p>Para desenvolvedores: se depende de JavaScript, valide a operação por teclado antes de ignorar.</p></div>`,
