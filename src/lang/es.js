...
const why = {
 ...
 <p>Solo procure no cambiar ambas cosas al mismo tiempo. Cambiar "Reproducir" a "Pausar, activado" significa que el reproductor está en pausa, no reproduciendo.</p></div>`,
	...
	links: `<div class="why"><p>Las personas recorren la página leyendo los enlaces y usando la búsqueda interna para encontrarlos por nombre, por lo que los enlaces efectivos deben ser significativos, únicos y concisos:</p><ul><li>Ideal: "Aprende sobre <a href="https://webaim.org/techniques/hypertext/link_text">enlaces significativos</a>"</li><li>No único: "Haz clic <a href="https://webaim.org/techniques/hypertext/link_text">aquí</a> para aprender sobre enlaces significativos."</li><li>No conciso: "<a href="https://webaim.org/techniques/hypertext/link_text">Haz clic aquí para aprender más sobre enlaces significativos</a>"</li></ul></div>`,

	imageLinks: `<div class="why"><p>El propósito del texto alternativo es proporcionar una alternativa al significado de una imagen, no a su contenido superficial. En imágenes enlazadas, el significado es la destino del enlace:<ul><li>"<em>Una lupa</em>" describe una imagen, no un enlace.</li><li>"<em>Una lupa de búsqueda</em>" describe ambiguamente ambas cosas.</li><li>"<em>Buscar</em>" describe correctamente el destino del enlace.</li></ul></p></div>`
};

const tips = {
	ALT_FILE_EXT: `<p>Los lectores de pantalla leerán esta URL, a menudo letra por letra. Esto probablemente no transmite el mismo significado que ver la imagen.</p><p>${why.fix}Agregue un alt vacío (alt="") si se trata de una decoración sin significado que deba ser ignorada por los lectores de pantalla, o agregue un texto alternativo descriptivo.</p>${why.images}`,

	ALT_MAYBE_BAD: `<p>Descripción proporcionada de esta imagen: <strong>"%(alt)"</strong></p><p>${why.fix}Establezca el texto alternativo de esta imagen como una descripción concisa de lo que significa en este contexto.</p>${why.images}`,

	ALT_PLACEHOLDER: `<p>Descripción proporcionada de esta imagen: <strong>"%(alt)"</strong></p><p>${why.fix}Establezca el texto alternativo de esta imagen como una descripción concisa de lo que significa en este contexto.</p>${why.images}`,

	ALT_UNPRONOUNCEABLE: `<p>El texto alternativo de esta imagen es "%(alt)", y solo contiene símbolos impronunciables y/o espacios. Los lectores de pantalla anunciarán que hay una imagen y luego harán una pausa incómoda: "imagen: ____".</p><p>${why.fix}Agregue un alt descriptivo, o proporcione un alt <em>completamente</em> vacío (alt="") si esto es solo un ícono o separador que los lectores de pantalla deben ignorar.</p>${why.images}`,

	BTN_EMPTY: `<p>${why.fix}Use cualquier método válido para indicar a los lectores de pantalla qué hace este botón; por ejemplo: texto, texto alternativo en un ícono o el atributo <code>title</code>.</p>`,
	...
	HIDDEN_FOCUSABLE: `Este elemento interactivo tiene un atributo <code>aria-hidden="true"</code>, pero sigue siendo accesible mediante teclado. Si <strong>pretende</strong> ocultar este elemento a los lectores de pantalla, también debe agregar <code>tabindex="-1"</code>. De lo contrario, elimine el atributo <code>aria-hidden="true"</code>.`,

	IMAGE_ALT_TOO_LONG: `<p>Los textos alternativos se leen como una sola oración continua; si alguien se pierde algo, debe volver a escucharlo completo.</p><p>El texto alternativo de %(altLength) caracteres de esta imagen es: <em class="ed11y-small">%(ALT_TEXT)</em></p><div class="why"><p>Consejo: las imágenes complejas que transmiten más información de la que cabe en una frase generalmente necesitan un pie de foto <strong>visible</strong> o una alternativa que describa o interprete los detalles clave. Está bien remitir a las personas usuarias de lector de pantalla a ese texto:</p><ul><li>"Póster del baile del viernes; detalles en el pie de foto."</li><li>"Gráfico que muestra que los casos han bajado un 10% este año; detalles en la tabla."</li></ul></div>`,
	...
	LABELS_MISSING_LABEL: `No hay una etiqueta asociada con este campo. Agregue un atributo <code>id</code> a este campo y un atributo <code>for</code> correspondiente en la etiqueta.`,
	...
	LINK_SUS_ALT: `<p>El alt de esta imagen incluye la palabra "%(alt)", lo que generalmente indica que no describe el destino del enlace.</p><strong class="badge">Texto alternativo</strong> "%(ALT_TEXT)"<p>Para corregirlo: asegúrese de que el alt describa la finalidad o el destino del enlace.<br></p>${why.imageLinks}`,

	LINK_SYMBOLS: `${why.fix}Evite usar símbolos como llamadas a la acción dentro del texto del enlace a menos que estén ocultos a las tecnologías de asistencia. Los lectores de pantalla pueden leerlos en voz alta, lo que puede confundir. Considere eliminar: <strong {C}>%(ERROR)</strong>`,
