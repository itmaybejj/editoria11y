 ...
 * injects ed11y in headless mode, and checks whether ed11y's results are
...
const ED11Y_UMD_PATH = join(PROJECT_ROOT, 'dist', 'js', 'ed11y.umd.js');
const ED11Y_LANG_PATH = join(PROJECT_ROOT, 'dist', 'js', 'lang', 'en.umd.js');

// Load test case index and ed11y source files
let testcases;
let ed11ySource;
let ed11yLangSource;
...
test.beforeAll(async () => {
  ...
  ed11ySource = await readFile(ED11Y_UMD_PATH, 'utf-8');
  ed11yLangSource = await readFile(ED11Y_LANG_PATH, 'utf-8');

  await mkdir(PARTS_DIR, { recursive: true });
});

/**
 * Inject ed11y into the page and run it in headless mode.
 *
 ...
 * a <body> exists since ed11y scans it by default.
 */
async function runEd11yOnPage(page, ruleOptions = {}) {
  const options = {
    ...conformanceOptions,
    ...ruleOptions,
    checks: {
  ...
  const results = await page.evaluate(
    ({ umdSource, langSource, opts }) => {
      ...
      // Inject ed11y UMD bundle via indirect eval (works without <head>)
      (0, eval)(umdSource);
      (0, eval)(langSource);
      ...
      return new Promise((resolve) => {
        ...
        document.addEventListener('ed11yResults', (e) => {
        ...
        try {
          if (typeof Ed11y === 'undefined') {
            clearTimeout(timeout);
            resolve({ results: [], error: 'Ed11y not loaded' });
            return;
          }
          Ed11y.Lang.addI18n(Ed11yLangEn.strings);
          new Ed11y.Ed11y(opts);
        } catch (err) {
          clearTimeout(timeout);
    ...
    { umdSource: ed11ySource, langSource: ed11yLangSource, opts: options },
  );
...
/**
 * Evaluate consistency between ed11y results and ACT expected outcome.
 */
function evaluateConsistency(expected, checkKeys, results) {
  ...
  // "passed" or "inapplicable" — ed11y should NOT have flagged
  return {
    consistent: matchingResults.length === 0,
    outcome: matchingResults.length === 0 ? 'earl:passed' : 'earl:failed',
    matchedKeys,
      ...
      for (const tc of ruleCases) {
        ...
        if (isNonHtml) {
          ...
          try {
            ...
            const runResult = await runEd11yOnPage(
              page,
              ruleConfig.options || {},
            );
        ...
        const details = failedCases
          .map(
            (o) =>
              ...
              `ed11y keys: [${o.allResultKeys.join(', ')}])`,
          )
          .join('\n');
