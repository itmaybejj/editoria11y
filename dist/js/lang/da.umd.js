(function(global, factory) {
  ...
  const tips = {
    ...
    HIDDEN_FOCUSABLE: `<p><strong>Element:</strong> <code>%(ELEMENT)</code></p><p>Dette interaktive element har <code>aria-hidden="true"</code>, men kan stadig fokuseres med tastatur. Skal det skjules for skærmlæsere, tilføj også <code>tabindex="-1"</code>; ellers fjern <code>aria-hidden</code>.</p>`,
    ...
    LINK_IMAGE_ALT: `Sørg for at denne alt‑tekst beskriver linkets destination:</p><p> {L} {ALT} <i>%(ALT_TEXT)</i></p>${why.imageLinks}`,
    ...
    LINK_MAYBE_BUTTON: `<p>Dette link har et ugyldigt mål og indeholder ordet <i>%(NAME)</i>. Det tyder på, at det måske slet ikke er et link, men i stedet styrer en scriptet funktion på siden.</p><p>${why.fix}erstat linket med en <a href="https://www.w3.org/WAI/ARIA/apg/patterns/button/">tilgængelig knap</a>, eller ret linkets destination.</p><div class="why"><p>Tip: Hjælpemidler behandler knapper og links forskelligt. Brug af det korrekte HTML‑element sikrer, at brugerne kender de rigtige tastaturgenveje og ved, hvilken handling der udløses.</p></div>`,
    ...
    QA_IN_PAGE_LINK: `<p><strong>ID:</strong> <code>%(ID)</code></p><p>Linkets mål matcher ikke noget element på siden.</p><div class="why"><p>Udviklernote: hvis det er en JS‑anker, så test tastaturbetjening før udelukkelse.</p></div>`,
    ...
    SUS_ALT: `<p>Denne alt‑tekst indeholder ordet <i>%(alt)</i>, hvilket sandsynligvis er overflødigt.</p><p>Alt‑tekst: <i>%(ALT_TEXT)</i></p><p><strong class="badge">Løsning</strong> Omskriv alt‑teksten, så den kort formidler billedets betydning.</p><div class="why"><p>Tip: skærmlæsere annoncerer, at de beskriver et billede, før de læser alt‑teksten op, så udtryk som "billede af" og "foto af" er ofte overflødige — medmindre ordene refererer til noget inde i billedet:</p><ul><li>Ikke overflødigt: <em>Et foto i</em> et fotoalbum, der vises for klassen.</li><li>Overflødigt: <em>Foto af</em> et foto i et fotoalbum.</li></ul></div>`,
