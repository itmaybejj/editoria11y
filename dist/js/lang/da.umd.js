(function(global, factory) {
  ...
  const tips$1 = {
    ...
    ALT_MAYBE_BAD: `<p>Alt‑tekst: <strong>"%(alt)"</strong></p><p><strong class="badge">Løsning</strong> Sæt billedets alternative tekst til en kort og præcis beskrivelse af, hvad billedet betyder i denne sammenhæng.</p>${why$1.images}`,
    ALT_PLACEHOLDER: `<p>Alt‑tekst: <strong>"%(alt)"</strong></p><p><strong class="badge">Løsning</strong> Sæt billedets alternative tekst til en kort og præcis beskrivelse af, hvad billedet betyder i denne sammenhæng.</p>${why$1.images}`,
    ...
    BTN_EMPTY_LABELLEDBY: `<p>Knappens <code>aria-labelledby</code> er tom eller matcher ikke et <code>ID</code> på siden.</p><p>${why$1.fix}Kobl ID’et til et eksisterende element eller fjern attributten og beskriv knappen på en anden måde.</p>`,
    ...
    DUPLICATE_ID: `<p>ID’er bruges her som etiketter eller linkmål og skal derfor være unikke.</p><p>${why$1.fix}Redigér denne ID: <strong>#%(id)</strong></p><div class="why"><p>I mange CMS stammer den fra et felt “name” eller “id”. I HTML er det attributten: <code>&lt;a id=""></code></p></div>`,
    ...
    EMBED_MISSING_TITLE: `<p>Indlejrede elementer kræver et tilgængeligt navn, der beskriver indholdet.</p><p>${why$1.fix}Tilføj et unikt <code>title</code> eller <code>aria-label</code>.</p>`,
    ...
    EMBED_VIDEO: `<p>Videoer skal have undertekster.</p><p>Automatiske undertekster skal korrigeres manuelt.</p><p>${why$1.fix}Tilføj/korrigér undertekster og luk derefter advarslen.</p>`,
    HEADING_EMPTY: `<p>Tomme overskrifter skaber huller i sidens struktur.</p><p>${why$1.fix}Tilføj tekst eller fjern den tomme linje.</p>${why$1.headings}`,
    HEADING_EMPTY_WITH_IMAGE: `<p>Tomme overskrifter skaber huller i sidens struktur.</p><p>${why$1.fix}Hvis det ikke er en overskrift, skift format fra <strong {C}>Overskrift %(level)</strong> til <strong>Afsnit</strong>. Ellers hør hjemme i billedets alt‑tekst.</p>${why$1.headings}`,
    HEADING_FIRST: `${why$1.fix}Sørg for, at sidetitel er markeret som Overskrift 1 eller Overskrift 2. ${why$1.headings}`,
    HEADING_LONG: `<p>${why$1.fix}Hvis ikke det er en fast titel (fx en udgivet artikel), så gør den kortere for bedre skimning.</p>${why$1.headings}`,
    HEADING_MISSING_ONE: `<p>${why$1.fix}Markér sidetitlen som overskrift på niveau 1 for at markere dokumentstart.</p>${why$1.headings}`,
    HEADING_SKIPPED_LEVEL: `<p>Overskriften sprang fra <strong>niveau %(prevLevel)</strong> til <strong>niveau %(level)</strong>; for skærmlæsere lyder det som manglende indhold.</p><p>${why$1.fix}Juster niveauerne, så strukturen er uden huller.</p>${why$1.headings}`,
    ...
    IMAGE_DECORATIVE: `<p>Billedet er skjult for skærmlæsere via tom alt. Kun reelt dekorative billeder bør skjules.</p><p>${why$1.fix}Hvis billedet har betydning, tilføj alt‑tekst.</p>${why$1.images}`,
    ...
    IMAGE_FIGURE_DECORATIVE: `<p>Billedet ignoreres af hjælpemidler. Giver billedteksten mening uden selve billedet?</p><p>${why$1.fix}Hvis ikke, tilføj alt‑tekst for det, billedteksten ikke dækker.</p><div class="why"><p>Tip: billeder, alt‑tekster og billedtekster arbejder sammen:</p><ul><li>Billedtekster giver kontekst/fortolkning.</li><li>Alt‑tekster beskriver billedet for dem, der ikke ser det.</li></ul></div>`,
    IMAGE_FIGURE_DUPLICATE_ALT: `<p>${why$1.fix}Redigér alt‑teksten, så den beskriver den visuelle betydning.</p><div class="why"><p>Tip: billeder, alt‑tekster og billedtekster arbejder sammen:</p><ul><li>Billedtekster leverer kontekst.</li><li>Alt‑tekster gør billedteksten forståelig for ikke‑seende.</li></ul></div>`,
    ...
    LABELS_INPUT_RESET: `<p>Nulstillingsknapper aktiveres let ved et uheld og kan medføre datatab.</p><p>${why$1.fix}Medmindre kun ét felt nulstilles, fjern knappen eller kræv bekræftelse.</p>`,
    ...
    LABELS_PLACEHOLDER: `<p>Placeholder‑tekst forsvinder under indtastning og kan have lav kontrast eller ligne indhold.</p><p>${why$1.fix}Sørg for, at vigtig info (label, hjælp, formatkrav) altid er synlig.</p>`,
    ...
    LINK_ALT_MAYBE_BAD: `<p>Alt‑tekst: "<strong>%(alt)</strong>"</p><p><strong class="badge">Løsning</strong> Sæt denne billedes alt‑tekst til linkdestinationens navn.</p>${why$1.imageLinks}`,
    LINK_ALT_UNPRONOUNCEABLE: `<p>Alt‑teksten for dette linkede billede består kun af uudtalelige tegn: “%(ALT_TEXT)”.</p><p>${why$1.fix}Sæt alt‑teksten til linkets mål eller formål.</p>${why$1.imageLinks}`,
    LINK_CLICK_HERE: "“Klik”/“Klik her” er overflødigt og skjuler linkets formål.",
    LINK_DOI: `<p>${why$1.fix}Brug artiklens titel som link og lad DOI stå som tekst, ikke omvendt.</p><div class="why"><p>Beskrivende links er nemmere at skimme efter navn og mere tilgængelige.</p><p>Skærmlæsere kan dermed annoncere meningsfulde links fremfor talrækker.</p></div>`,
    LINK_EMPTY: `<p>${why$1.fix}Tilføj tekst, der beskriver dens destination, eller slet den, hvis det bare er en stavefejl eller et linket mellemrum.</p><div class="why"><p>Tip: Skærmlæsere kan ikke beskrive links, der kun indeholder mellemrum eller symboler. De bliver enten tavse ("Link, [...ubehagelig pause hvor linktitlen burde være...]"), eller de læser URL'en: Link, H-T-T-P-S slash slash eksempel punktum com.</p><p>Bemærk at linkede mellemrum kan være svære at slette i nogle indholdseditorer; det er nogle gange nødvendigt at slette "på tværs af hullet" ved at fjerne og omskrive ordene på begge sider af et linket mellemrum.</p></div>`,
    LINK_EMPTY_LABELLEDBY: `<p>Linkets <code>aria-labelledby</code> henviser ikke til et eksisterende <code>ID</code>.</p><p>${why$1.fix}Angiv et gyldigt ID eller fjern attributten.</p>`,
    ...
    LINK_FILE_EXT: `<p>Linket fører til en fil (PDF, MP3, Zip, Word, …) uden advarsel.</p><p>${why$1.fix}Angiv filtype i linket via tekst/ikon.</a>.</p><p class="why">Angiv evt. filstørrelse: “Årsrapport (PDF, 3 MB)”.</p>`,
    LINK_IDENTICAL_NAME: `<p>Linktekst: "<strong>%(TEXT)</strong>"</p><p><strong class="badge">Løsning</strong> Omskriv links, der fører til forskellige steder, så deres tekst entydigt afspejler deres destination.</p>${why$1.links}`,
    LINK_IMAGE_ALT: `Sørg for at denne alt‑tekst beskriver linkets destination:</p><p> {L} {ALT} <strong {C}>%(ALT_TEXT)</strong></p>${why$1.imageLinks}`,
    LINK_IMAGE_ALT_AND_TEXT: `<p><span style="display: none;">%(ALT)</span>Link med alt‑tekst:<br>"<strong>%(LINK)</strong>"</p><p><strong class="badge">Løsning</strong> Ændr eller fjern alt‑teksten, hvis den tilføjer irrelevant eller gentaget information.</p>${why$1.imageLinks}`,
    ...
    LINK_IMAGE_NO_ALT_TEXT: `<p>Når et link indeholder et billede, bruges billedets alt‑tekst https://webaim.org/techniques/hypertext/link_text#alt_linksom linkets navn</p><p>${why$1.fix}Sæt alt‑teksten til linkets mål/formål.</p>${why$1.imageLinks}`,
    LINK_IMAGE_TEXT: "Billedet er markeret som dekorativt, men linket bruger omkringstående tekst som etiket.",
    LINK_NEW_TAB: `<p>${why$1.fix}Åbn i samme fane eller <a href="https://itmaybejj.github.io/linkpurpose/">advar brugeren</a> om ny fane.</p><div class="why"><p>Brugere kan selv vælge ny fane; tvang kan forvirre, især mht. “tilbage”.</p><p>I formularer åbner links ofte i ny fane for at undgå datatab.</p></div>`,
    LINK_PLACEHOLDER_ALT: `<p>Alt‑teksten for dette linkede billede er en placeholder: “<strong>%(alt)</strong>”.</p><p>${why$1.fix}Sæt alt‑teksten til linkets destination.</p>${why$1.imageLinks}`,
    LINK_STOPWORD: `<p>Linktekst: "<strong>%(text)</strong>"</p><p><strong class="badge">Løsning</strong> Brug linktekst, der beskriver destinationen.</p>${why$1.links}`,
    LINK_STOPWORD_ARIA: `<p>ARIA leverer et tilgængeligt navn, men den synlige tekst er generisk: “<strong {C}>%(ERROR)</strong>”.</p><p>${why$1.fix}Skriv meningsfulde, synlige linktekster, der matcher ARIA‑navnet.</p>${why$1.links}`,
    LINK_SUS_ALT: `<p>Denne alt‑tekst indeholder ordet "%(alt)", hvilket normalt betyder, at den ikke beskriver linkets destination.</p><p>Alt‑tekst: "<strong>%(ALT_TEXT)</strong>"</p><p><strong class="badge">Løsning</strong> Sørg for, at alt‑teksten beskriver linkets formål eller destination.</p>${why$1.imageLinks}`,
    ...
    LINK_URL: `<p>${why$1.fix}Erstat URL som linktekst med titel eller formål.</p><div class="why"><p>Brugere — særligt skærmlæser‑brugere — skimmer links ved navn.</p><p>URL’er er svære at skimme/søge i.</p></div>`,
    META_LANG: `<p>${why$1.fix}Tilføj <a href="https://www.w3.org/International/questions/qa-html-language-declarations">sprog‑attribut</a> på sidens HTML‑tag.</p><div class="why"><p>Skærmlæsere vælger udtale efter sprog. Forkert sprog ⇒ uforståelig tale.</p></div>`,
    META_MAX: `<p>Dette meta‑tag begrænser zoom.</p><p>${why$1.fix}Fjern/tilpas begrænsningen, så fuld zoom er mulig.</p>`,
    META_REFRESH: `<p>Automatisk opdatering via meta afbryder brugeren og kan nulstille formularer.</p><p>${why$1.fix}Brug AJAX/JS og giv varsel, så brugeren kan udsætte opdateringen.</p>`,
    META_SCALABLE: `<p>Dette meta‑tag forhindrer zoom.</p><p>${why$1.fix}Tillad fuld zoom ved at fjerne/justere tagget.</p>`,
    ...
    MISSING_ALT: `<p>Mangler alt, læser skærmlæsere billed‑URL’en bogstav for bogstav.</p><p>${why$1.fix}Brug alt="" for dekorative billeder, eller en beskrivende alt‑tekst for meningsbærende billeder.</p>${why$1.images}`,
    MISSING_ALT_LINK: `<p>Ved linkede billeder uden alt læses billed‑URL’en — særligt problematisk.</p><p>${why$1.fix}Tilføj en alt‑tekst, der beskriver linkets mål.</p>${why$1.imageLinks}`,
    MISSING_ALT_LINK_HAS_TEXT: `<p>Billedet er del af et link med tekst. Hvis synlig tekst er tilstrækkelig, brug alt="". Ellers tilføj alt‑tekst, der beskriver målet.</p>${why$1.imageLinks}`,
    QA_BAD_LINK: `<p>Linket ser ud til at pege på et udviklingsmiljø:<br>{L} <strong {C}>%(LINK)</strong></p><p>${why$1.fix}Brug en relativ sti (/folder) eller offentlig URL.</p>`,
    ...
    QA_FAKE_HEADING: `<p>${why$1.fix}Hvis den fede linje introducerer et emne, så brug overskriftsformat.</p><div class="why"><p>Tip: overskrifter giver en navigérbar indholdsfortegnelse for hjælpemidler.</p></div>`,
    ...
    QA_JUSTIFY: `<p>Fuldjusteret tekst skaber ujævne mellemrum og gør læsning vanskelig.</p><p>${why$1.fix}Brug venstrejusteret tekst.</p>`,
    ...
    QA_PDF: `<p>${why$1.fix}Gør én af følgende og luk advarslen:</p><ul><li>Link til en webside i stedet for PDF,</li><li>eller tilbyd også en web/redigerbar version,</li><li>eller sørg for at PDF’en er tagget (overskrifter, læserækkefølge, tabelhoveder, alt‑tekster).</li></ul><div class="why"><p>Mange — især mobil‑ og hjælpemiddel‑brugere — foretrækker websider frem for PDF’er.</p></div>`,
    QA_SMALL_TEXT: "For lille tekst er svær at læse, særligt for brugere med nedsat syn. Undgå mindre end standardstørrelse.",
    QA_STRONG_ITALICS: `<p>${why$1.fix}Brug fed/kursiv sparsomt til nøgleord.</p><div class="why"><p>Tip: er det et citat, så brug <code>blockquote</code>.</p></div>`,
    QA_SUBSCRIPT: `Hævet/sænket skrift gør teksten lille og svær at læse. Brug kun til fx 4<sup>e</sup>, H<sub>2</sub>O, fodnoter.`,
    QA_UNDERLINE: `<p>Understregning betyder “link” på webben. Brugere vil tro, det kan klikkes.</p><p>${why$1.fix}Brug <strong>fed</strong> eller <em>kursiv</em> til fremhævelse, og overskrifter til nye sektioner.</p><div class="why"><p>Skærmlæsere annoncerer ikke visuelle stilarter; struktur skabes af overskrifter.</p></div>`,
    QA_UPPERCASE: `<p>STORE BLOKKE MED VERSALER ER SVÆRERE AT LÆSE OG KAN OPFATTES SOM “RAAB”.</p><p>${why$1.fix}Fremhæv få ord ad gangen, helst med fed frem for versaler.</p><div class="why"><p>Skærmlæsere annoncerer ikke “fed”. Brug overskrift ved nyt emne.</p></div>`,
    ...
    TABLES_EMPTY_HEADING: ` <p>${why$1.fix}Sørg for, at hver tabeloverskriftscelle indeholder tekst.</p><div class="why"><p>Tip: skærmlæsere bruger overskrifter til at orientere i tabeller.</p></div>`,
    TABLES_MISSING_HEADINGS: ` <p>${why$1.fix}Angiv i tabellens indstillinger om overskrifter findes i første række, første kolonne eller begge.</p><div class="why"> <p>Tip: skærmlæsere gentager relevante overskrifter i celler.</p><p>Er tabellen kun til layout, fjern tabel‑formatet.</p></div>`,
    TABLES_SEMANTIC_HEADING: ` <p>${why$1.fix}Fjern indholdsoverskrifter (h2, h3) fra tabellen. Brug tabeloverskrifter. For flere niveauer: del i flere tabeller.</p><div class="why"> <p>Tip: tabeloverskrifter er rettede (række/kolonne). Indholdsoverskrifter gælder alt efterfølgende — også i andre kolonner:</p><table><tr><th>1</th><th>2</th><th>3</th><td rowspan="2">En <strong>tabeloverskrift</strong> i celle 2 mærker celle B. <br><br> En <strong>indholdsoverskrift</strong> i celle 2 påvirker 3, A, B, C og denne tekst samt tooltip‑fod.</td></tr> <tr><td>A</td><td>B</td><td>C</td></tr></table> </div>`
  };
  const interfaceStrings$1 = {
    ALERT_CLOSE: "Luk",
    ALT: "Alt‑tekst: ",
  ...
  ({
    strings: Object.assign(Sa11yStrings.strings, interfaceStrings$1, tips$1)
  });
  const Sa11yStrings = {
    // Danish
    strings: {
