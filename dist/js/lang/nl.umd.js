(function(global, factory) {
  ...
  const testNames = {
    ...
    TABLES_INVALID_HEADERS_REF: "Deze tabel heeft ongeldige koptekstreferenties",
  ...
  const tips = {
    ALT_FILE_EXT: `<p><span style="display: none">%(alt)</span><strong>Alt‑tekst:</strong> <i>%(ALT_TEXT)</i></p><p>Schermlezers lezen deze URL voor, vaak letter voor letter. Dit geeft meestal niet dezelfde betekenis als het beeld zelf.</p><p>${why.fix}Beschrijf beknopt wat deze afbeelding in deze context betekent.</p>${why.images}`,
    ...
    ARIA_INPUT_FIELD_NAME: `<p><strong>Element:</strong> <code>%(EL)</code></p><p>${why.fix}Geef een geldig label op; voor aangepaste invoerelementen betekent dat vaak inner‑tekst, of een title‑, aria-label‑ of aria-labelledby‑attribuut.`,
    ...
    LABEL_IN_NAME: `<p><strong>Zichtbare tekst:</strong> <i>%(TEXT)</i></p><p><strong>Label voor schermlezers:</strong> <i>%(TEXT)</i></p><p>De zichtbare tekst van dit element lijkt anders te zijn dan de toegankelijke naam. Dit kan schermlezergebruikers verwarren en spraakbesturing verstoren.</p><p>${why.fix}Zorg dat het zichtbare label begint met de tekst van het onzichtbare label en geen betekenisvolle informatie bevat die niet in het onzichtbare label staat.</p>`,
    LINK_ALT_FILE_EXT: `<p><span style="display: none;">%(ALT)</span>Alt‑tekst: <i>%(alt)</i></p><p>Deze alt‑tekst is waarschijnlijk een bestandsnaam in plaats van een betekenisvolle linktitel.</p><p>${why.fix}Gebruik de titel van de linkbestemming als alt‑tekst voor gelinkte afbeeldingen.</p><div class="why"><p>Alt‑tekst moet de betekenis van een afbeelding beschrijven. Voor een gelinkte afbeelding is dat de bestemming:</p><ul><li>"Pagina met tekst" beschrijft het beeld, niet de link.</li><li>"IMG_1234.jpg" is slechts een bestandsnaam.</li><li>"Formulier voor evenementregistratie (.doc)" is het linkdoel.</li></ul></p></div>`,
    ...
    LINK_STOPWORD: `<p>Linktekst: <i>%(text)</i></p><p><strong class="badge">Oplossen</strong> Gebruik linktekst die het doel beschrijft.</p>${why.links}`,
    LINK_STOPWORD_ARIA: `<p>Er is een ARIA‑naam aanwezig, maar de zichtbare tekst is generiek: <i>%(ERROR)</i>.</p><p>${why.fix}Zorg voor betekenisvolle zichtbare linktekst en laat die overeenkomen met de toegankelijke naam.</p>${why.links}`,
    ...
    MISSING_ALT: `<p>Zonder alt leest een schermlezer vaak de bestands‑URL letter voor letter voor.</p><p>${why.fix}Beschrijf beknopt wat deze afbeelding in deze context betekent.</p>${why.images}`,
