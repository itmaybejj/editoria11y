(function(global, factory) {
  ...
  const tips = {
    ...
    HIDDEN_FOCUSABLE: `<p><strong>Element:</strong> <code>%(ELEMENT)</code></p><p>Dit interactieve element heeft <code>aria-hidden="true"</code>, maar kan nog steeds focussen. Als u het echt wilt verbergen voor schermlezers, voeg dan <code>tabindex="-1"</code> toe; anders verwijdert u <code>aria-hidden</code>.</p>`,
    ...
    LABEL_IN_NAME: `<p><strong>Zichtbare tekst:</strong> <i>%(TEXT)</i></p><p><strong>Toegankelijke naam:</strong> <i>%(TEXT)</i></p><p>De zichtbare tekst van dit element lijkt anders te zijn dan de toegankelijke naam. Dit kan schermlezergebruikers verwarren en spraakbesturing verstoren.</p><p>${why.fix}Zorg dat het zichtbare label begint met de tekst van het onzichtbare label en geen betekenisvolle informatie bevat die niet in het onzichtbare label staat.</p>`,
    ...
    LINK_IMAGE_ALT: `Controleer of de alt‑tekst de linkbestemming beschrijft:</p><p> {L} {ALT} <i>%(ALT_TEXT)</i></p>${why.imageLinks}`,
    ...
    QA_IN_PAGE_LINK: `<p><strong>ID:</strong> <code>%(ID)</code></p><p>Het linkdoel komt niet overeen met een element op deze pagina.</p><div class="why"><p>Voor ontwikkelaars: als dit door JavaScript wordt afgehandeld, verifieer dan toetsenbordbediening voordat u deze controle negeert.</p></div>`,
