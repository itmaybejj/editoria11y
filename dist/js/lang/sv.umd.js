(function(global, factory) {
  ...
  const testNames = {
    ...
    LINK_MAYBE_BUTTON: "Den här länken ser ut att egentligen vara en knapp",
  ...
  const tips = {
    ...
    HIDDEN_FOCUSABLE: `<p><strong>Element:</strong> <code>%(ELEMENT)</code></p><p>Detta interaktiva element har <code>aria-hidden="true"</code> men kan fortfarande fokuseras. Om det ska döljas för skärmläsare, lägg till <code>tabindex="-1"</code>; annars ta bort <code>aria-hidden</code>.</p>`,
    ...
    LABEL_IN_NAME: `<p><strong>Synlig text:</strong> <i>%(TEXT)</i></p><p><strong>Tillgängligt namn:</strong> <i>%(TEXT)</i></p><p>Den synliga texten i detta element verkar skilja sig från det tillgängliga namnet. Detta kan förvirra skärmläsaranvändare och påverka röststyrning.</p><p>${why.fix}Se till att den synliga etiketten börjar med texten i den osynliga etiketten och inte innehåller någon betydelsefull information som saknas i den osynliga etiketten.</p>`,
    ...
    LINK_IMAGE_ALT: `<p>Kontrollera att alt‑texten beskriver länkens mål:</p><p> {L} {ALT} <i>%(ALT_TEXT)</i></p>${why.imageLinks}`,
    ...
    LINK_MAYBE_BUTTON: `<p>Denna länk har ett ogiltigt mål och innehåller ordet <i>%(NAME)</i>. Det tyder på att detta kanske inte är en länk utan i stället styr ett skriptat beteende på sidan.</p><p>${why.fix}ersätt länken med en <a href="https://www.w3.org/WAI/ARIA/apg/patterns/button/">tillgänglig knapp</a>, eller korrigera länkens mål.</p><div class="why"><p>Tips: Hjälpmedelstekniker behandlar knappar och länkar olika. Att använda rätt HTML‑element säkerställer att användare vet vilka kortkommandon de ska använda och vilken åtgärd som utlöses.</p></div>`,
    ...
    QA_IN_PAGE_LINK: `<p><strong>ID:</strong> <code>%(ID)</code></p><p>Länkmålet för denna interna länk saknas på sidan.</p><div class="why"><p>För utvecklare: om JS hanterar länken, säkerställ tangentbordsstöd innan varningen ignoreras.</p></div>`,
