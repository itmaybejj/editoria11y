(function(global, factory) {
  ...
  const testNames = {
    ...
    META_LANG_VALID: "Språkkoden för detta element är inte giltig",
  ...
  const tips = {
    ...
    ALT_FILE_EXT: `<p><span style="display: none">%(alt)</span>Alt‑text: <i>%(ALT_TEXT)</i></p><p>Skärmläsare läser upp denna URL, ofta tecken för tecken. Det ger sällan samma mening som att se bilden.</p><p>${why.fix}Lägg till alt="" om detta är ren dekoration som bör ignoreras av skärmläsare, eller lägg till en beskrivande alt‑text.</p>${why.images}`,
    ALT_MAYBE_BAD: `<p>Alt‑text: <i>%(alt)</i></p><p>${why.fix}Ange en alt‑text som kort beskriver vad bilden betyder i detta sammanhang.</p>${why.images}`,
    ALT_MAYBE_BAD_WARNING: `<p>Alt‑text: <i>%(alt)</i></p><p>${why.fix}Ange en alt‑text som kort beskriver vad bilden betyder i detta sammanhang.</p>${why.images}`,
    ALT_PLACEHOLDER: `<p>Alt‑text: <i>%(alt)</i></p><p>${why.fix}Ange en alt‑text som kort beskriver vad bilden betyder i detta sammanhang.</p>${why.images}`,
    ALT_UNPRONOUNCEABLE: `<p>Alt‑text: <i>%(alt)</i></p><p>Denna alt‑text innehåller endast tecken som inte kan uttalas eller bara mellanslag. Skärmläsare kommer att meddela att en bild finns och sedan göra en onaturlig paus eller läsa något oförståeligt.</p><p>${why.fix}Lägg till en beskrivande alt‑text, eller använd alt="" om detta bara är en ikon eller ett mellanrum som bör ignoreras.</p>${why.images}`,
    ...
    LINK_ALT_FILE_EXT: `<p><span style="display: none;">%(ALT)</span>Alt‑text: <i>%(alt)</i></p><p>Denna alt‑text är troligen ett filnamn i stället för ett meningsfullt länk‑namn.</p><p>${why.fix}Ange bildens alt‑text som namnet på länkens mål.</p><div class="why"><p>Alt‑text ska beskriva vad en bild betyder. För en länkad bild är betydelsen målet:</p><ul><li>"Sida med text" beskriver bilden, inte länken.</li><li>"IMG_1234.jpg" är bara ett filnamn.</li><li>"Anmälningsformulär för evenemang (.doc)" är länkens mål.</li></ul></p></div>`,
    LINK_ALT_MAYBE_BAD: `<p>Alt‑text: <i>%(alt)</i></p><p>${why.fix}Ange alt‑texten som namnet på länkens mål.</p>${why.imageLinks}`,
    LINK_ALT_MAYBE_BAD_WARNING: `<p>Alt‑text: <i>%(alt)</i></p><p>${why.fix}Ange alt‑texten som namnet på länkens mål.</p>${why.imageLinks}`,
    LINK_ALT_UNPRONOUNCEABLE: `<p>Alt‑texten <i>%(ALT_TEXT)</i> består av tecken som inte kan uttalas; länken kan inte beskrivas.</p><p>${why.fix}Ange alt som beskriver länkens syfte eller mål.</p>${why.imageLinks}`,
    ...
    LINK_IMAGE_ALT: `<p><strong>Alt‑text:</strong> <i>%(ALT_TEXT)</i></p><p>${why.fix}Se till att denna alt‑text beskriver länkens mål, och inte bara bildens visuella innehåll:</p>${why.imageLinks}`,
    ...
    LINK_IMAGE_LONG_ALT: `<p>Alt‑text på %(altLength) tecken: <i class="ed11y-small">%(ALT_TEXT)</i></p><p>${why.fix}Ange denna bilds alt‑text som titeln för länkens mål.</p><div class="why"><p>Skärmläsaranvändare hör ofta en lista med länkar utan sammanhang. Eftersom alt‑texten för en länkad bild blir länkens titel bör den beskriva länkens mål, inte bildens innehåll.</p></div>`,
    ...
    LINK_PLACEHOLDER_ALT: `<p>Alt‑texten är en platshållare: <i>%(alt)</i>.</p><p>${why.fix}Ange alt som beskriver länkens mål.</p>${why.imageLinks}`,
    ...
    LINK_SUS_ALT: `<p>Denna bilds alt‑text innehåller ordet <i>%(alt)</i>, vilket oftast betyder att den inte beskriver länkens mål.</p><p>Alt‑text: <i>%(ALT_TEXT)</i></p><p>${why.fix}Se till att alt‑texten beskriver länkens mål eller syfte.</p>${why.imageLinks}`,
    ...
    SUS_ALT: `<p>Denna alt‑text innehåller ordet <i>%(alt)</i>:</p><p><i>%(ALT_TEXT)</i></p><p>${why.fix}Skriv om alt‑texten så att den kort anger vad bilden betyder.</p><div class="why"><p>Tips: skärmläsare meddelar att de beskriver en bild innan alt‑texten läses, så fraser som "bild av" och "foto av" är vanligtvis överflödiga</p></div>`,
