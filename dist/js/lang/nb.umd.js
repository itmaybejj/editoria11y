(function(global, factory) {
  ...
  const tips = {
    ...
    BTN_ROLE_IN_NAME: `<p><strong>Tilgjengelig navn:</strong> <i>%(TEXT)</i></p><p>Skjermlesere bruker ordet «button» for å annonsere at de beskriver en knapp, så dette ordet er overflødig.</p><p>${why.fix}Knappens etikett skal samsvare med dens handling. Hvis den synlige etiketten er et ikon i stedet for tekst, merk knappen med ikonets betydning, f.eks. «Spill», «Søk» eller «Meny».</p>`,
    BTN_TIP: `${why.buttons}`,
    ...
    HIDDEN_FOCUSABLE: `<p><strong>Element:</strong> <code>%(ELEMENT)</code></p><p>Dette interaktive elementet har <code>aria-hidden="true"</code>, men kan få tastaturfokus. Hvis du vil skjule det for skjermlesere, bruk også <code>tabindex="-1"</code>; hvis ikke, fjern <code>aria-hidden</code>.</p>`,
    ...
    LINK_IMAGE_ALT: `Kontroller at alt‑teksten beskriver lenkens mål:</p><p> {L} {ALT} <i>%(ALT_TEXT)</i></p>${why.imageLinks}`,
    ...
    QA_IN_PAGE_LINK: `<p><strong>ID:</strong> <code>%(ID)</code></p><p>Målet for denne interne lenken finnes ikke på siden.</p><div class="why"><p>For utviklere: Hvis lenken håndteres av JavaScript, verifiser tastaturstøtte.</p></div>`,
