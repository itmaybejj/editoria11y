(function(global, factory) {
  ...
  const Sa11yStrings = {
    ...
    strings: {
      ...
      TABLES_INVALID_HEADERS_REF: '<code>headers</code>-attributtet inneholder en ugyldig referanse: <strong {C}>%(VALUE)</strong>. Det må referere til ID-en til en gyldig overskriftscelle i samme tabell. <hr> Lær mer om <a href="https://www.w3.org/WAI/WCAG22/Techniques/html/H43">bruk av id- og headers-attributter for å koble dataceller til overskriftsceller i datatabeller.</a>',
      // Contrast
  ...
  const testNames = {
    ...
    TABLES_INVALID_HEADERS_REF: "Denne tabellen har ugyldige overskriftsreferanser",
  ...
  const tips = {
    ARIA_INPUT_FIELD_NAME: `<p><strong>Element:</strong> <code>%(EL)</code></p><p>${why.fix}Oppgi en gyldig etikett; for egendefinerte inndataelementer betyr det ofte indre tekst, eller et title-, aria-label- eller aria-labelledby-attributt.`,
    ALT_FILE_EXT: `<p><span style="display: none">%(alt)</span><strong>Alt-tekst:</strong> <i>%(ALT_TEXT)</i></p><p>Skjermlesere vil lese opp denne URL-en, ofte bokstav for bokstav. Det gir vanligvis ikke samme mening som å se bildet.</p><p>${why.fix}Beskriv kort hva dette bildet betyr, i denne konteksten.</p>${why.images}`,
    ...
    BTN_ROLE_IN_NAME: `<p><strong>Etikett for skjermlesere:</strong> <i>%(TEXT)</i></p><p>Skjermlesere bruker ordet «button» for å annonsere at de beskriver en knapp, så dette ordet er overflødig.</p><p>${why.fix}Knappens etikett skal samsvare med dens handling. Hvis den synlige etiketten er et ikon i stedet for tekst, merk knappen med ikonets betydning, f.eks. «Spill», «Søk» eller «Meny».</p>`,
    BTN_TIP: `${why.buttons}`,
    ...
    EMBED_UNFOCUSABLE: `<p>Dette attributtet ber tastatur og hjelpemidler hoppe over elementet. Med mindre innholdet i denne iframen ikke inneholder lenker, knapper eller skjemaelementer og ikke kan rulles, må attributtet fjernes.</p>`,
    ...
    LINK_STOPWORD: `<p>Lenketekst: <i>%(text)</i></p><p><strong class="badge">Slik løser du det</strong> Bruk lenketekst som beskriver målet.</p>${why.links}`,
    LINK_STOPWORD_ARIA: `<p>Lenken har en ARIA‑etikett, men den synlige teksten er generell: <i>%(ERROR)</i>.</p><p>${why.fix}Gi meningsfull synlig lenketekst, og sørg for samsvar med det tilgjengelige navnet.</p>${why.links}`,
    ...
    MISSING_ALT: `<p>Uten alt leser skjermlesere opp bilde‑URL (ofte bokstavvis).</p><p>${why.fix}Beskriv kort hva dette bildet betyr, i denne konteksten.</p>${why.images}`,
