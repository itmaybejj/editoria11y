(function(global, factory) {
  ...
  const tips = {
    ALT_FILE_EXT: `<p><span hidden>%(alt)</span>Alt テキスト：<strong>"%(ALT_TEXT)"</strong></p><p>スクリーンリーダーはこの URL を一文字ずつ読み上げることがあり、画像を見たときと同じ意味にはなりません。</p><p><strong class="badge">修正方法</strong> 無意味な装飾でスクリーンリーダーに無視させたい場合は alt=""（空の alt）を、意味がある場合は説明的な alt を追加してください。</p>${why.images}`,
    ...
    LINK_FILE_EXT: `<p>このリンクは PDF／MP3／ZIP／Word などのファイルに遷移しますが、その旨の表示がありません。</p><p>${why.fix}リンクテキスト内で <a href="https://itmaybejj.github.io/linkpurpose/">ファイル形式を明示</a>してください（テキストやアイコンなど）。</p><p class="why">大きいファイルはサイズも明示すると親切です（例：「年次報告書（PDF、3 MB）」）。</p>`,
    ...
    LINK_IMAGE_ALT: `<p>この alt がリンクの目的を説明しているか確認してください：</p><p> {L} {ALT} <strong {C}>%(ALT_TEXT)</strong></p>${why.imageLinks}`,
    LINK_IMAGE_ALT_AND_TEXT: `<p>Alt を含むリンク：<br>"<strong>%(LINK)</strong>"</p><p><strong class="badge">修正方法</strong> alt が不要または冗長な場合は修正または削除してください。</p>${why.imageLinks}`,
    ...
    LINK_MAYBE_BUTTON: `<p>このリンクには無効なリンク先があり、「<strong>%(NAME)</strong>」という語が含まれています。これはリンクではなく、ページ上でスクリプトによる動作を制御するものである可能性があります。</p><p>${why.fix}リンクを<a href="https://www.w3.org/WAI/ARIA/apg/patterns/button/">アクセシブルなボタン</a>に置き換えるか、リンク先を修正してください。</p><div class="why"><p>ヒント：支援技術はボタンとリンクを異なる方法で扱います。正しい HTML 要素を使用することで、ユーザーが適切なキーボードショートカットを使い、どのアクションが実行されるかを理解できます。</p></div>`,
    LINK_NEW_TAB: `<p>${why.fix}同じタブで開くようにするか、新しいタブで開くことを <a href="https://itmaybejj.github.io/linkpurpose/">事前に知らせてください</a>。</p><div class="why"><p>ユーザーは自分で「新しいタブで開く」ことを選べます。強制すると混乱のもとになります（「戻る」が期待通りに動作しない等）。</p><p>注：フォーム内のリンクは、入力内容の消失を避けるために新しいタブで開くことがあります。</p></div>`,
