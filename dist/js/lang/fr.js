...
const tooltip = {
  ...
  LINK_UNPRONOUNCEABLE: "Le texte du lien ne contient que des symboles. Si vous pensez que ce lien est une erreur due à un problème de copier-coller, envisagez de le supprimer.",
  // Images
  ...
  EMBED_MISSING_TITLE: 'Le contenu intégré requiert un nom accessible qui décrit le contenu. Veuillez fournir un titre (<code>title</code>) unique ou <code>aria-label</code> un attribut à l’élément <code>&lt;%(ELEMENT)&gt;</code>. En savoir plus sur les <a href="https://developer.mozilla.org/fr/docs/Web/HTML/Element/iframe#une_iframe_simple">iFrames.</a>',
  ...
  LABEL_IN_NAME: "Le texte visible pour cet élément semble différent du nom accessible, ce qui peut entraîner de la confusion pour les utilisateurs des technologies d’assistance. Veuillez vérifier : <hr> <strong {B}>Texte</strong> <strong {C}>%(TEXT)</strong> <hr> <strong {B}>Nom Accessible</strong> <strong {C}>%(TEXT)</strong>",
...
const interfaceStrings = {
  ALERT_CLOSE: "Fermer",
  ALT: "Texte alternatif : ",
  CONSOLE_ERROR: `Un problème affecte le vérificateur d’accessibilité sur cette page. Veuillez <a class="g-link">le signaler sur GitHub</a>. Informations de débogage :`,
