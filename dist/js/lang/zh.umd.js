(function(global, factory) {
  ...
  const tips = {
    ALT_FILE_EXT: `<p><span style="display: none">%(alt)</span>替代文本：<i>%(ALT_TEXT)</i></p><p>读屏软件会逐字逐字符朗读此 URL，无法传达与看到图片相同的含义。</p><p>${why.fix}若此图像为无意义装饰，请使用空 alt（alt=""）；若为内容图像，请提供描述其意义的替代文本。</p>${why.images}`,
    ALT_MAYBE_BAD: `<p>替代文本：<i>%(alt)</i></p><p>${why.fix}将替代文本改为简要且符合当前语境的描述，表达图片的意义。</p>${why.images}`,
    ALT_MAYBE_BAD_WARNING: `<p>替代文本：<i>%(alt)</i></p><p>${why.fix}将替代文本改为简要且符合当前语境的描述，表达图片的意义。</p>${why.images}`,
    ALT_PLACEHOLDER: `<p>替代文本：<i>%(alt)</i></p><p>${why.fix}将替代文本改为简要且符合当前语境的描述，表达图片的意义。</p>${why.images}`,
    ALT_UNPRONOUNCEABLE: `<p>替代文本：<i>%(alt)</i></p><p>此替代文本仅包含无法发音的符号和/或空格。读屏软件会提示有图像存在，然后出现尴尬的停顿或读出无法理解的内容。</p><p>${why.fix}请添加描述性替代文本；若此图像仅为图标或占位，请提供空 alt（alt=""）。</p>${why.images}`,
    ...
    IMAGE_ALT_TOO_LONG: `<p>替代文本长度：%(altLength) 字符：<i class="ed11y-small">%(ALT_TEXT)</i></p><p>${why.fix}请总结内容，或将部分描述移至图像下方的可见说明中。</p><div class="why"><p>提示：复杂图像往往无法用一句话完整表达。这类图像通常需要<strong>可见的</strong>描述性说明。可以在替代文本中引导读屏用户查看该说明：</p><ul><li>"周五舞会的海报；详见下方说明。"</li><li>"图表显示本年度问题数量下降 10%；详细信息见数据表。"</li></ul></div>`,
    ...
    LINK_ALT_FILE_EXT: `<p><span style="display: none;">%(ALT)</span>替代文本：<i>%(alt)</i></p><p>此替代文本可能是文件名，而非有意义的链接标签。</p><p>${why.fix}请将替代文本改为链接目标的名称。</p><div class="why"><p>替代文本应表达图像的意义。对于作为链接的图像，其意义是链接要前往的目标：</p><ul><li>"带文字的页面" 描述的是图像，而非链接。</li><li>"IMG_1234.jpg" 只是文件名。</li><li>"活动报名表（.doc）" 描述的是链接目标。</li></ul></p></div>`,
    LINK_ALT_MAYBE_BAD: `<p>替代文本：<i>%(alt)</i></p><p>${why.fix}请将替代文本设置为链接目标的名称。</p>${why.imageLinks}`,
    LINK_ALT_MAYBE_BAD_WARNING: `<p>替代文本：<i>%(alt)</i></p><p>${why.fix}请将替代文本设置为链接目标的名称。</p>${why.imageLinks}`,
    LINK_ALT_UNPRONOUNCEABLE: `<p>该链接图片的替代文本仅包含不可发音的符号和/或空格：<i>%(ALT_TEXT)</i></p><p>读屏软件会宣布存在链接，但无法描述其含义。</p><p>${why.fix}请将替代文本设置为链接的目标或用途。</p>${why.imageLinks}`,
    ...
    LINK_IMAGE_ALT: `<p><strong>替代文本：</strong> <i>%(ALT_TEXT)</i></p><p>${why.fix}请确保该 alt 描述链接目标，而非仅仅图像的视觉内容：</p>${why.imageLinks}`,
    ...
    LINK_IMAGE_LONG_ALT: `<p>替代文本长度 %(altLength) 字符：<i class="ed11y-small">%(ALT_TEXT)</i></p><p>${why.fix}将替代文本设置为链接目标的标题。</p><div class="why"><p>读屏用户常在无上下文时阅读页面链接列表。因为图像链接的替代文本会成为该列表中的链接名称，它应描述链接的目标，而不是图像内容。</p></div>`,
    ...
    LINK_PLACEHOLDER_ALT: `<p>该链接图片的替代文本为占位符：<i>%(alt)</i>。</p><p>${why.fix}请将替代文本改为链接目标的名称。</p>${why.imageLinks}`,
    ...
    LINK_SUS_ALT: `<p>此替代文本包含 <i>%(alt)</i>，通常意味着它未准确描述链接目标。</p><p>替代文本：<i>%(ALT_TEXT)</i></p><p>${why.fix}请确保替代文本表达链接的目的或作用。</p>${why.imageLinks}`,
    ...
    SUS_ALT: `<p>此替代文本包含 <i>%(alt)</i>：</p><p><i>%(ALT_TEXT)</i></p><p>${why.fix}重写替代文本，使其简要传达图像的意义。</p><div class="why"><p>提示：读屏软件在朗读替代文本前会告知用户“图像”，因此“图片”“照片”等词通常是冗余的</p></div>`,
