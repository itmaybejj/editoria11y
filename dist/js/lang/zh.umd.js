(function(global, factory) {
  ...
  const testNames = {
    ...
    LINK_LABEL: "链接标签",
    LINK_MAYBE_BUTTON: "此链接看起来应该是一个按钮",
  ...
  const tips = {
    ...
    BTN_TIP: `${why.buttons}`,
    BTN_ROLE_IN_NAME: `<p><strong>可访问名称：</strong> <i>%(TEXT)</i></p><p>读屏软件使用“按钮”一词来宣布正在描述一个按钮，因此这个词是多余的。</p><p>${why.fix}按钮的标签应与其行为相符。如果可见标签是图标而非文字，请按图标的含义为按钮命名，例如“播放”、“搜索”或“菜单”。</p>`,
    ...
    HIDDEN_FOCUSABLE: `<p><strong>元素：</strong> <code>%(ELEMENT)</code></p><p>该交互元素带有 <code>aria-hidden="true"</code>，但仍可通过键盘聚焦。若<strong>确实</strong>要对读屏软件隐藏，则还需添加 <code>tabindex="-1"</code>；否则请移除 <code>aria-hidden</code>。</p>`,
    ...
    LABEL_IN_NAME: `<p><strong>可见文本：</strong> <i>%(TEXT)</i></p><p><strong>可访问名称：</strong> <i>%(TEXT)</i></p><p>该元素的可见文本似乎与其可访问名称不同。这可能导致读屏用户困惑，或导致语音控制失败。</p><p>${why.fix}请确保可见标签以不可见标签的文本开头，且不包含任何在不可见标签中缺失的重要信息。</p>`,
    ...
    LINK_IMAGE_ALT: `请确保该 alt 描述链接目标：</p><p> {L} {ALT} <i>%(ALT_TEXT)</i></p>${why.imageLinks}`,
    ...
    LINK_MAYBE_BUTTON: `<p>此链接的目标无效，且包含<i>%(NAME)</i>一词。这表明它可能并非真正的链接，而是控制页面上某些脚本行为。</p><p>${why.fix}请将该链接替换为<a href="https://www.w3.org/WAI/ARIA/apg/patterns/button/">可访问的按钮</a>，或修正链接的目标地址。</p><div class="why"><p>提示：辅助技术对按钮和链接的处理方式不同。使用正确的 HTML 元素可确保用户了解应使用哪些键盘快捷键以及将触发何种操作。</p></div>`,
    ...
    QA_IN_PAGE_LINK: `<p><strong>ID:</strong> <code>%(ID)</code></p><p>该链接的目标与本页任何元素均不匹配。</p><div class="why"><p>给开发者的提示：若这是用于触发 JavaScript 的伪链接，请先测试其是否可通过键盘触发，再将其加入忽略名单。</p></div>`,
    ...
    SUS_ALT: `<p>此替代文本包含 <i>%(alt)</i>，可能是冗余的。</p><p>替代文本：<i>%(ALT_TEXT)</i></p><p>${why.fix}重写替代文本，使其简要传达图像的意义。</p><div class="why"><p>提示：读屏软件在朗读替代文本前会告知用户“图像”，因此“图片：”“照片：”等词通常冗余，除非它们描述的是图像内部的重要内容：</p><ul><li>不冗余：展示给全班的相册中的<em>一张照片</em>。</li><li>冗余：相册中的照片的<em>照片</em>。</li></ul></div>`,
