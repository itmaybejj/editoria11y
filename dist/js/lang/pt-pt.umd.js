(function(global, factory) {
  typeof exports === "object" && typeof module !== "undefined" ? factory(exports) : typeof define === "function" && define.amd ? define(["exports"], factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, factory(global["Ed11yLangPt-pt"] = {}));
  ...
  const tooltip = {
    ...
    EMBED_MISSING_TITLE: 'O conteúdo incorporado requer um nome acessível que descreva o seu conteúdo. Forneça um atributo <code>title</code> ou <code>aria-label</code> exclusivo no elemento <code>&lt;%(ELEMENT)&gt;</code>. Saiba mais sobre <a href="https://web.dev/learn/accessibility/more-html#iframes">iFrames.</a>',
  ...
  const testNames = {
    ...
    IMAGE_FIGURE_DECORATIVE: "Esta imagem com legenda não tem texto alternativo",
    ...
    LINK_CLICK_HERE: "Esta ligação contém “clique aqui”",
    ...
    LINK_IMAGE_ALT: "Este texto alternativo descreve a ligação ou a imagem?",
    ...
    LINK_IMAGE_TEXT: "Esta imagem usada como ligação precisa de uma descrição?",
    ...
    LINK_SYMBOLS: "Os símbolos ou emoji nesta ligação são significativos?",
    ...
    QA_BAD_LINK: "O destino desta ligação pode ser inválido",
  ...
  const tips = {
    ...
    BTN_UNPRONOUNCEABLE: `<p><strong>Texto do botão:</strong> <i>%(TEXT)</i></p><p>${why.fix}Adicione texto, um título ou um aria-label descrevendo o seu destino.</p><div class="why"><p>Dica: os leitores de ecrã não conseguem descrever botões que contêm apenas espaços ou símbolos. Ficam silenciosos (“Botão, [...pausa incómoda onde o rótulo do botão deveria estar...]”), ou leem o nome do símbolo.</p></div>`,
    ...
    EMBED_GENERAL: 'Ferramentas automáticas não analisam conteúdo dentro de embeds. Garanta alt em imagens, legendas em vídeos, contraste suficiente e ligações/botões <a href="https://webaim.org/techniques/keyboard/">acessíveis por teclado</a>. Depois, ignore o alerta.',
    ...
    HEADING_UNPRONOUNCEABLE: `<p>Cabeçalhos vazios criam lacunas na estrutura.</p><p>${why.fix}Adicione texto legível a este cabeçalho, ou converta-o num parágrafo.</p>${why.headings}`,
    ...
    HIDDEN_FOCUSABLE: `<p>Este elemento tem <code>aria-hidden="true"</code> mas recebe foco por teclado.</p><p>${why.fix}Para o esconder de leitores, adicione <code>tabindex="-1"</code>; caso contrário, remova <code>aria-hidden</code>.</p><p><strong>Elemento:</strong> <code>%(ELEMENT)</code></p>`,
    ...
    LINK_FILE_EXT: `<p><strong>Texto da ligação:</strong> <i>%(TEXT)</i></p><p>Esta ligação abre um ficheiro (PDF/MP3/ZIP/DOC) sem aviso.</p><p>${why.fix}Identifique o tipo de ficheiro no próprio texto/ícone da ligação: <a href="https://itmaybejj.github.io/linkpurpose/"></a>.</p><p class="why">Para ficheiros grandes, indique o tamanho (ex.: “Relatório (PDF, 3 MB)”).</p>`,
    LINK_IDENTICAL_NAME: `<p><strong>Texto da ligação:</strong> <i>%(TEXT)</i></p><p>${why.fix}Reformule as ligações que apontam para destinos diferentes, de forma a que o texto reflicta o título único de cada destino.</p>${why.links}`,
    ...
    LINK_IMAGE_NO_ALT_TEXT: `<p>Quando a ligação contém uma imagem, o alt torna-se <a href="https://webaim.org/techniques/hypertext/link_text#alt_link">o rótulo da ligação</a>.</p><p>${why.fix}Use o título do destino da ligação como texto alternativo para imagens usadas como ligação.</p>${why.imageLinks}`,
    ...
    LINK_NEW_TAB: `<p><strong>Texto da ligação:</strong> <i>%(TEXT)</i></p><p>${why.fix}Abra no mesmo separador ou <a href="https://itmaybejj.github.io/linkpurpose/">avise antecipadamente</a> que abrirá em novo separador.</p><div class="why"><p>Forçar novo separador pode confundir (o botão “voltar” comporta-se de forma diferente). Em formulários, às vezes evita perda de dados.</p></div>`,
    ...
    META_LANG: `<p>${why.fix}Adicione <a href="https://www.w3.org/International/questions/qa-html-language-declarations">o atributo de idioma</a> ao elemento <code>html</code>.</p><div class="why"><p>Leitores de ecrã dependem do idioma para pronunciar corretamente; idioma incorreto prejudica a compreensão.</p></div>`,
    ...
    META_TITLE: `<p>${why.fix}Inclua <code>&lt;title&gt;</code> dentro de <code>head</code>.</p><div class="why"><p>Um <a href="https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/title">título curto e único</a> é essencial: motores de busca, separadores e leitores de ecrã dependem dele.</p><p>Sem título, o utilizador vê/ouve apenas a URL.</p></div>`,
    ...
    QA_IN_PAGE_LINK: `<p><strong>URL:</strong> <i>#%(ID)</i></p><p><strong>Ligação:</strong> <code>%(TEXT)</code></p><p>O destino desta âncora interna não existe na página.</p><div class="why"><p>Para programadores: se depende de JavaScript, valide o funcionamento por teclado antes de ignorar.</p></div>`,
