(function(global, factory) {
  ...
  const testNames = {
    ...
    LINK_MAYBE_BUTTON: "Esta ligação parece que deveria ser um botão",
  ...
  const tips = {
    ...
    BTN_TIP: `${why.buttons}`,
    BTN_ROLE_IN_NAME: `<p><strong>Nome acessível:</strong> <i>%(TEXT)</i></p><p>Os leitores de ecrã utilizam a palavra "button" para anunciar que estão a descrever um botão, pelo que esta palavra é redundante.</p><p>${why.fix}O rótulo do botão deve corresponder à sua ação. Se o rótulo visível for um ícone em vez de texto, rotule o botão com o significado do ícone, por exemplo, "Reproduzir", "Pesquisar" ou "Menu".</p>`,
    ...
    HIDDEN_FOCUSABLE: `<p><strong>Elemento:</strong> <code>%(ELEMENT)</code></p><p>Este elemento tem <code>aria-hidden="true"</code> mas recebe foco por teclado. Para o esconder de leitores, adicione <code>tabindex="-1"</code>; caso contrário, remova <code>aria-hidden</code>.</p>`,
    ...
    LABEL_IN_NAME: `<p><strong>Texto visível:</strong> <i>%(TEXT)</i></p><p><strong>Nome acessível:</strong> <i>%(TEXT)</i></p><p>O texto visível deste elemento parece ser diferente do seu nome acessível. Isto pode confundir utilizadores de leitores de ecrã e prejudicar o controlo por voz.</p><p>${why.fix}Garanta que a legenda visível começa pelo texto da legenda invisível e não contém qualquer informação significativa em falta na legenda invisível.</p>`,
    ...
    LINK_MAYBE_BUTTON: `<p>Esta ligação tem um destino inválido e contém a palavra <i>%(NAME)</i>. Isto sugere que pode não ser uma ligação, mas sim que controla algum comportamento programático na página.</p><p>${why.fix}substitua a ligação por um <a href="https://www.w3.org/WAI/ARIA/apg/patterns/button/">botão acessível</a>, ou corrija o destino da ligação.</p><div class="why"><p>Sugestão: as tecnologias de apoio tratam botões e ligações de forma diferente. Usar o elemento HTML correto garante que os utilizadores sabem quais atalhos de teclado utilizar e que ação será desencadeada.</p></div>`,
    ...
    QA_IN_PAGE_LINK: `<p><strong>ID:</strong> <code>%(ID)</code></p><p>O destino desta âncora interna não existe na página.</p><div class="why"><p>Para programadores: se depende de JavaScript, valide o funcionamento por teclado antes de ignorar.</p></div>`,
