(function(global, factory) {
  ...
  const testNames = {
    ...
    LINK_MAYBE_BUTTON: "Questo link sembra essere un pulsante",
...
<li>Ha segnato il gol della vittoria dalla fascia sinistra!</li></ul></div>`,
  ...
  const tips = {
    ...
    HIDDEN_FOCUSABLE: `<p><strong>Elemento:</strong> <code>%(ELEMENT)</code></p><p>Questo elemento ha <code>aria-hidden="true"</code>, ma è ancora focalizzabile da tastiera. Se va nascosto agli screen reader, aggiungi <code>tabindex="-1"</code>; altrimenti rimuovi <code>aria-hidden</code>.</p>`,
    ...
    LABEL_IN_NAME: `<p><strong>Testo visibile:</strong> <i>%(TEXT)</i></p><p><strong>Nome accessibile:</strong> <i>%(TEXT)</i></p><p>Il testo visibile di questo elemento sembra diverso dal nome accessibile. Questo può confondere gli utenti di lettori di schermo e compromettere il controllo vocale.</p><p>${why.fix}Assicurati che l’etichetta visibile inizi con il testo dell’etichetta invisibile e non contenga informazioni significative che manchino nell’etichetta invisibile.</p>`,
    ...
    LINK_IMAGE_ALT: `Assicurati che l’alt descriva la destinazione del link:</p><p> {L} {ALT} <i>%(ALT_TEXT)</i></p>${why.imageLinks}`,
    ...
    LINK_MAYBE_BUTTON: `<p>Questo link ha una destinazione non valida e contiene la parola <i>%(NAME)</i>. Ciò suggerisce che potrebbe non essere un link, ma un elemento che controlla un comportamento scriptato nella pagina.</p><p>${why.fix}sostituisci il link con un <a href="https://www.w3.org/WAI/ARIA/apg/patterns/button/">pulsante accessibile</a>, oppure correggi la destinazione del link.</p><div class="why"><p>Suggerimento: le tecnologie assistive trattano pulsanti e link in modo diverso. Usare l’elemento HTML corretto garantisce che gli utenti sappiano quali scorciatoie da tastiera utilizzare e quale azione verrà attivata.</p></div>`,
    ...
    QA_IN_PAGE_LINK: `<p><strong>ID:</strong> <code>%(ID)</code></p><p>La destinazione del link non corrisponde a nessun elemento in pagina.</p><div class="why"><p>Nota dev: se è un handler JS, verifica anche l’uso da tastiera prima di escludere il controllo.</p></div>`,
    ...
    SUS_ALT: `<p>Questo alt contiene la parola <i>%(alt)</i>, probabilmente ridondante.</p><p>Alt: <i>%(ALT_TEXT)</i></p><p><strong class="badge">Correzione</strong> Riscrivi l’alt per esprimere brevemente il significato.</p><div class="why"><p>I lettori annunciano già “immagine”; frasi come “immagine di” o “foto di” sono spesso superflue.</p><p>Eccezione se descrivono contenuto interno:</p><ul><li>Non ridondante: <em>Una foto in</em> un album mostrato alla classe.</li><li>Ridondante: <em>Foto di</em> una foto in un album mostrato alla classe.</li></ul></div>`,
