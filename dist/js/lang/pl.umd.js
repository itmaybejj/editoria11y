(function(global, factory) {
  ...
  const tips = {
    ...
    BTN_EMPTY: `<p>${why.fix}Użyj dowolnej poprawnej metody, aby przekazać czytnikom ekranu funkcję tego przycisku — tekst, alt na ikonie lub atrybut title.</p>`,
    ...
    HIDDEN_FOCUSABLE: `<p>Element ma <code>aria-hidden="true"</code>, a mimo to może otrzymać fokus klawiatury.</p><p>${why.fix}Jeśli ma być niewidoczny dla czytników, dodaj też <code>tabindex="-1"</code>; w przeciwnym razie usuń <code>aria-hidden</code>.</p><p><strong>Element:</strong> <code>%(ELEMENT)</code></p>`,
    ...
    LABELS_MISSING_LABEL: `<p>${why.fix}Dodaj atrybut <code>id</code> do tego pola oraz odpowiadający mu atrybut <code>for</code> do etykiety.</p>`,
    ...
    LINK_EMPTY_NO_LABEL: `<p>${why.fix}Dodaj tekst opisujący cel linku albo usuń link, jeśli to przypadkowy błąd.</p><div class="why"><p>Wskazówka: czytniki ekranu nie potrafią odczytać pustych linków; milkną lub odczytują URL.</p><p>Linkowane spacje mogą być trudne do usunięcia; usuń tekst z obu stron i wpisz go ponownie.</p></div>`,
    ...
    LINK_IDENTICAL_NAME: `<p><strong>Tekst linku:</strong> <i>%(TEXT)</i></p><p>${why.fix}Zmień treść linków prowadzących do różnych miejsc tak, aby ich teksty były unikalne.</p>${why.links}`,
    ...
    LINK_STOPWORD: `<p>Tekst linku: <i>%(ERROR)</i></p><p>${why.fix}Użyj tekstu linku, który opisuje jego cel.</p>${why.links}`,
