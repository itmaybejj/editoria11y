(function(global, factory) {
  ...
  const tips = {
    ...
    ALT_FILE_EXT: `<p><span style="display: none">%(alt)</span><strong>Tekst alternatywny:</strong> <i>%(ALT_TEXT)</i></p><p>Czytniki ekranu odczytają ten adres URL, często litera po literze. Prawdopodobnie nie odda to znaczenia obrazu.</p><p>${why.fix}Krótko opisz, co ten obraz oznacza w danym kontekście.</p>${why.images}`,
    ...
    BTN_ROLE_IN_NAME: `<p><strong>Etykieta dla czytników ekranu:</strong> <i>%(TEXT)</i></p><p>Czytniki ekranu używają słowa „button”, aby zapowiedzieć, że opisują przycisk, więc to słowo jest zbędne.</p><p>${why.fix}Etykieta przycisku powinna odpowiadać jego działaniu. Jeśli widoczną etykietą jest ikona, a nie tekst, opisz przycisk zgodnie ze znaczeniem ikony, np. „Odtwórz”, „Szukaj” lub „Menu”.</p>`,
    BTN_TIP: `${why.buttons}`,
    ...
    LABEL_IN_NAME: `<p><strong>Widoczny tekst:</strong> <i>%(TEXT)</i></p><p><strong>Etykieta dla czytników ekranu:</strong> <i>%(TEXT)</i></p><p>Widoczny tekst tego elementu różni się od jego dostępnej nazwy. Może to wprowadzać w błąd użytkowników czytników ekranu oraz zakłócać sterowanie głosowe.</p><p>${why.fix}Upewnij się, że widoczna etykieta zaczyna się od tekstu niewidocznej etykiety i nie zawiera żadnych istotnych informacji, których brakuje w niewidocznej etykiecie.</p>`,
    LINK_ALT_FILE_EXT: `<p><span style="display: none;">%(ALT)</span>Tekst alternatywny: <i>%(alt)</i></p><p>Ten tekst alternatywny prawdopodobnie jest nazwą pliku, a nie znaczącą nazwą linku.</p><p>${why.fix}Jako tekstu alternatywnego obrazów‑linków używaj tytułu celu linku.</p><div class="why"><p>Cel tekstu alternatywnego to opisanie, co obraz znaczy. W przypadku obrazu będącego linkiem oznacza to cel linku:</p><ul><li>"Strona z tekstem" opisuje obraz, nie link.</li><li>"IMG_1234.jpg" to tylko nazwa pliku.</li><li>"Formularz zgłoszeniowy wydarzenia (.doc)" opisuje cel linku.</li></ul></p></div>`,
    ...
    LINK_STOPWORD: `<p>Tekst linku: <i>%(text)</i></p><p><strong class="badge">Aby naprawić</strong> Użyj tekstu linku, który opisuje jego cel.</p>${why.links}`,
    LINK_STOPWORD_ARIA: `<p>Jest nazwa ARIA, ale widoczny tekst jest ogólny: <i>%(ERROR)</i>.</p><p>${why.fix}Zadbaj o znaczącą widoczną etykietę i spójność z ARIA.</p>${why.links}`,
    ...
    MISSING_ALT: `<p>Bez alt czytnik odczyta URL obrazu (często znak po znaku).</p><p>${why.fix}Krótko opisz, co ten obraz oznacza w danym kontekście.</p>${why.images}`,
