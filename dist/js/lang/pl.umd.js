(function(global, factory) {
  ...
  const tips = {
    ...
    HIDDEN_FOCUSABLE: `<p><strong>Element:</strong> <code>%(ELEMENT)</code></p><p>Element ma <code>aria-hidden="true"</code>, a mimo to może otrzymać fokus klawiatury. Jeśli ma być niewidoczny dla czytników, dodaj też <code>tabindex="-1"</code>; w przeciwnym razie usuń <code>aria-hidden</code>.</p>`,
    ...
    LABEL_IN_NAME: `<p><strong>Widoczny tekst:</strong> <i>%(TEXT)</i></p><p><strong>Nazwa dostępna:</strong> <i>%(TEXT)</i></p><p>Widoczny tekst tego elementu różni się od jego dostępnej nazwy. Może to wprowadzać w błąd użytkowników czytników ekranu oraz zakłócać sterowanie głosowe.</p><p>${why.fix}Upewnij się, że widoczna etykieta zaczyna się od tekstu niewidocznej etykiety i nie zawiera żadnych istotnych informacji, których brakuje w niewidocznej etykiecie.</p>`,
    ...
    LINK_IMAGE_ALT: `<p>Upewnij się, że alt opisuje cel linku:</p><p> {L} {ALT} <i>%(ALT_TEXT)</i></p>${why.imageLinks}`,
    ...
    QA_IN_PAGE_LINK: `<p><strong>ID:</strong> <code>%(ID)</code></p><p>Cel linku wewnętrznego nie istnieje na stronie.</p><div class="why"><p>Dla programistów: jeśli to handler JS — zweryfikuj obsługę klawiaturą, zanim wyłączysz kontrolę.</p></div>`,
