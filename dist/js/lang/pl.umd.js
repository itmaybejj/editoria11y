(function(global, factory) {
  ...
  const testNames = {
    ...
    META_LANG_VALID: "Kod języka tego elementu jest nieprawidłowy",
  ...
  const tips = {
    ...
    ALT_FILE_EXT: `<p><span style="display: none">%(alt)</span>Tekst alternatywny: <i>%(ALT_TEXT)</i></p><p>Czytniki ekranu odczytają ten adres URL, często litera po literze. Prawdopodobnie nie odda to znaczenia obrazu.</p><p><strong class="badge">Aby naprawić</strong> Jeśli to dekoracja, dodaj puste alt (alt=""), aby czytniki ekranu ją pominęły; jeśli nie, dodaj opisowy tekst alternatywny.</p>${why.images}`,
    ALT_MAYBE_BAD: `<p>Tekst alternatywny: <i>%(alt)</i></p><p><strong class="badge">Aby naprawić</strong> Ustaw tekst alternatywny jako krótką, adekwatną do kontekstu informację, opisującą znaczenie obrazu.</p>${why.images}`,
    ALT_MAYBE_BAD_WARNING: `<p>Tekst alternatywny: <i>%(alt)</i></p><p><strong class="badge">Aby naprawić</strong> Ustaw tekst alternatywny jako krótką, adekwatną do kontekstu informację, opisującą znaczenie obrazu.</p>${why.images}`,
    ALT_PLACEHOLDER: `<p>Tekst alternatywny: <i>%(alt)</i></p><p><strong class="badge">Aby naprawić</strong> Ustaw tekst alternatywny jako krótką, adekwatną do kontekstu informację, opisującą znaczenie obrazu.</p>${why.images}`,
    ALT_UNPRONOUNCEABLE: `<p>Tekst alternatywny: <i>%(alt)</i></p><p>Ten tekst alternatywny zawiera tylko niewymawialne symbole lub spacje. Czytniki ekranu ogłoszą obecność obrazu, po czym zrobią niezręczną pauzę lub odczytają niezrozumiały ciąg.</p><p><strong class="badge">Aby naprawić</strong> Dodaj opisowy tekst alternatywny lub użyj alt="" (całkowicie pustego), jeśli to jedynie ikona albo element odstępu do pominięcia przez czytniki.</p>${why.images}`,
    ...
    LINK_ALT_FILE_EXT: `<p><span style="display: none;">%(ALT)</span>Tekst alternatywny: <i>%(alt)</i></p><p>Ten tekst alternatywny prawdopodobnie jest nazwą pliku, a nie znaczącą nazwą linku.</p><p><strong class="badge">Aby naprawić</strong> Ustaw tekst alternatywny obrazu jako nazwę celu linku.</p><div class="why"><p>Cel tekstu alternatywnego to opisanie, co obraz znaczy. W przypadku obrazu będącego linkiem oznacza to cel linku:</p><ul><li>"Strona z tekstem" opisuje obraz, nie link.</li><li>"IMG_1234.jpg" to tylko nazwa pliku.</li><li>"Formularz zgłoszeniowy wydarzenia (.doc)" opisuje cel linku.</li></ul></p></div>`,
    LINK_ALT_MAYBE_BAD: `<p>Tekst alternatywny: <i>%(alt)</i></p><p><strong class="badge">Aby naprawić</strong> Ustaw tekst alternatywny obrazu na nazwę celu linku.</p>${why.imageLinks}`,
    LINK_ALT_MAYBE_BAD_WARNING: `<p>Tekst alternatywny: <i>%(alt)</i></p><p><strong class="badge">Aby naprawić</strong> Ustaw tekst alternatywny obrazu na nazwę celu linku.</p>${why.imageLinks}`,
    LINK_ALT_UNPRONOUNCEABLE: `<p>Alt <i>%(ALT_TEXT)</i> zawiera niewymawialne znaki/odstępy — linku nie da się opisać.</p><p>${why.fix}Opisz w alcie cel lub funkcję linku.</p>${why.imageLinks}`,
    ...
    LINK_IMAGE_ALT: `<p><strong>Tekst alternatywny:</strong> <i>%(ALT_TEXT)</i></p><p>${why.fix}Upewnij się, że ten alt opisuje cel linku, a nie tylko wizualną zawartość obrazu:</p>${why.imageLinks}`,
    ...
    LINK_PLACEHOLDER_ALT: `<p>Alt w obrazie‑linku to placeholder: <i>%(alt)</i>.</p><p>${why.fix}Ustaw alt na cel linku.</p>${why.imageLinks}`,
    ...
    LINK_SUS_ALT: `<p>Tekst alternatywny obrazu zawiera słowo "%(alt)", co zwykle oznacza, że nie opisuje celu linku.</p><p>Tekst alternatywny: <i>%(ALT_TEXT)</i></p><p><strong class="badge">Aby naprawić</strong> Upewnij się, że tekst alternatywny opisuje cel lub funkcję linku.</p>${why.imageLinks}`,
    ...
    SUS_ALT: `<p>Tekst alternatywny tego obrazu zawiera słowo <i>%(alt)</i>:</p><p><i>%(ALT_TEXT)</i></p><p><strong class="badge">Aby naprawić</strong> Przepisz tekst alternatywny tak, by krótko przekazywał znaczenie obrazu.</p><div class="why"><p>Wskazówka: czytniki ekranu informują użytkownika, że opisują obraz, więc zwroty typu "obraz przedstawiający" lub "zdjęcie przedstawiające" zwykle są zbędne</p><p>Uwaga: to jest w porządku, jeśli słowa odnoszą się do treści wewnątrz obrazu:</p><ul><li>Niezbędne: <em>Zdjęcie w</em> albumie pokazywanym klasie.</li><li>Zbędne: <em>Zdjęcie przedstawiające</em> zdjęcie w albumie.</li></ul></div>`,
