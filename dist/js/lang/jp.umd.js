(function(global, factory) {
  ...
  const tips = {
    ...
    BTN_TIP: `${why.buttons}`,
    BTN_ROLE_IN_NAME: `<p><strong>アクセシブルネーム：</strong> <i>%(TEXT)</i></p><p>スクリーンリーダーは「ボタン」という語を使ってボタンを説明していることを伝えるため、ラベル内の「button」は冗長になります。</p><p>${why.fix}ボタンのラベルは動作と一致させてください。可視ラベルがテキストではなくアイコンの場合は、アイコンの意味（例：「再生」「検索」「メニュー」）をラベルにしてください。</p>`,
    ...
    HIDDEN_FOCUSABLE: `<p><strong>要素：</strong> <code>%(ELEMENT)</code></p><p>このインタラクティブ要素は <code>aria-hidden="true"</code> が設定されていますが、キーボードでフォーカスできます。スクリーンリーダーから隠したい場合は <code>tabindex="-1"</code> を追加してください。隠さない場合は <code>aria-hidden</code> を削除してください。</p>`,
    ...
    LABEL_IN_NAME: `<p><strong>可視テキスト：</strong> <i>%(TEXT)</i></p><p><strong>アクセシブルネーム：</strong> <i>%(TEXT)</i></p><p>この要素の視覚的なテキストとアクセシブルネームが異なっている可能性があります。スクリーンリーダー利用者を混乱させたり、音声操作が正常に働かない可能性があります。</p><p>${why.fix}可視ラベルが非表示ラベルのテキストで始まり、非表示ラベルに含まれていない重要な情報が可視ラベルに含まれないようにしてください。</p>`,
    ...
    LINK_IMAGE_ALT: `この alt がリンクの目的を説明しているか確認してください：</p><p> {L} {ALT} <i>%(ALT_TEXT)</i></p>${why.imageLinks}`,
    ...
    QA_IN_PAGE_LINK: `<p><strong>ID:</strong> <code>%(ID)</code></p><p>このページ内リンクのターゲットが存在しません。</p><div class="why"><p>開発者向け：JavaScript で処理している場合は、キーボード操作でも動作することを確認してください。</p></div>`,
    ...
    SUS_ALT: `<p>この画像の alt には <i>%(alt)</i> という語が含まれており、冗長である可能性があります。</p><p>Alt テキスト：<i>%(ALT_TEXT)</i></p><p><strong class="badge">修正方法</strong> 画像が何を意味するのかを簡潔に伝える alt にしてください。</p><div class="why"><p>ヒント：スクリーンリーダーは alt を読む前に「画像」と伝えるため、「〜の画像」「〜の写真」といった語は通常不要です。ただし画像の内容自体を指す場合は例外です：</p><ul><li>冗長でない：<em>クラスに見せているアルバムの「写真」</em></li><li>冗長：<em>写真の</em> アルバムにある写真</li></ul></div>`,
    // REVIEW
