(function(global, factory) {
  ...
  const testNames = {
    ...
    META_LANG_VALID: "この要素の言語コードが有効ではありません",
  ...
  const tips = {
    ALT_FILE_EXT: `<p><span style="display: none">%(alt)</span>Alt テキスト：<i>%(ALT_TEXT)</i></p><p>スクリーンリーダーはこの URL を一文字ずつ読み上げることがあり、画像を見たときと同じ意味にはなりません。</p><p><strong class="badge">修正方法</strong> 無意味な装飾でスクリーンリーダーに無視させたい場合は alt=""（空の alt）を、意味がある場合は説明的な alt を追加してください。</p>${why.images}`,
    ALT_MAYBE_BAD: `<p>Alt テキスト：<i>%(alt)</i></p><p><strong class="badge">修正方法</strong> この画像がこの文脈で何を意味するのか、簡潔に説明する alt に設定してください。</p>${why.images}`,
    ALT_MAYBE_BAD_WARNING: `<p>Alt テキスト：<i>%(alt)</i></p><p><strong class="badge">修正方法</strong> この画像がこの文脈で何を意味するのか、簡潔に説明する alt に設定してください。</p>${why.images}`,
    ALT_PLACEHOLDER: `<p>Alt テキスト：<i>%(alt)</i></p><p><strong class="badge">修正方法</strong> この画像がこの文脈で何を意味するのか、簡潔に説明する alt に設定してください。</p>${why.images}`,
    ALT_UNPRONOUNCEABLE: `<p>Alt テキスト：<i>%(alt)</i></p><p>この alt テキストには発音できない記号や空白のみが含まれています。スクリーンリーダーは画像があることを知らせた後、不自然に間を置くか、意味のない読み上げになります。</p><p><strong class="badge">修正方法</strong> 説明的な alt を追加するか、ただのアイコンやスペーサーなど無視させたい場合は完全な空の alt (alt="") を指定してください。</p>${why.images}`,
    ...
    IMAGE_ALT_TOO_LONG: `<p>%(altLength) 文字の alt テキスト：<i class="ed11y-small">%(ALT_TEXT)</i></p><p><strong class="badge">修正方法</strong> 要約するか、一部の説明をキャプションに移してください。</p><div class="why"><p>ヒント：1 文に収まらない情報量のある複雑な画像は、重要な詳細を説明・解釈する<strong>目に見える</strong>キャプションや別の代替テキストが必要になることが多いです。スクリーンリーダー利用者をそのテキストに誘導しても構いません：</p><ul><li>"金曜のダンスのポスター。詳細はキャプションに記載。"</li><li>"今年の問題件数が 10% 減少したことを示すグラフ。詳細は表に記載。"</li></ul></div>`,
    ...
    LINK_ALT_MAYBE_BAD: `<p>Alt テキスト：<i>%(alt)</i></p><p><strong class="badge">修正方法</strong> この画像の alt をリンク先のタイトルに設定してください。</p>${why.imageLinks}`,
    LINK_ALT_MAYBE_BAD_WARNING: `<p>Alt テキスト：<i>%(alt)</i></p><p><strong class="badge">修正方法</strong> この画像の alt をリンク先のタイトルに設定してください。</p>${why.imageLinks}`,
    LINK_ALT_UNPRONOUNCEABLE: `<p>このリンク画像の alt は発音できない文字や空白のみです：<i>%(ALT_TEXT)</i></p><p>スクリーンリーダーはリンクがあることを知らせますが、説明できません。</p><p>${why.fix}リンクの目的（行き先・機能）を説明する alt にしてください。</p>${why.imageLinks}`,
    ...
    LINK_IMAGE_ALT: `<p><strong>Alt テキスト：</strong> <i>%(ALT_TEXT)</i></p><p>${why.fix}この alt が、画像の視覚的な内容だけでなくリンク先を説明しているか確認してください。</p>${why.imageLinks}`,
    ...
    LINK_IMAGE_LONG_ALT: `<p>%(altLength) 文字の alt テキスト：<i class="ed11y-small">%(ALT_TEXT)</i></p><p><strong class="badge">修正方法</strong> この画像の alt をリンク先のタイトルに設定してください。</p><div class="why"><p>スクリーンリーダー利用者は文脈なしでページ内のリンク一覧を聞くことがあります。リンク画像の alt はリンクのタイトルとして読み上げられるため、<a href="https://webaim.org/techniques/hypertext/link_text#alt_link">画像の内容ではなくリンク先の説明</a>である必要があります。</p></div>`,
    ...
    LINK_PLACEHOLDER_ALT: `<p>このリンク画像の alt はプレースホルダーです：<i>%(alt)</i></p><p>${why.fix}リンク先を説明する alt を設定してください。</p>${why.imageLinks}`,
    ...
    LINK_SUS_ALT: `<p>この画像の alt には <i>%(alt)</i> という単語が含まれており、リンク先を説明していない可能性があります。</p><p>Alt テキスト：<i>%(ALT_TEXT)</i></p><p><strong class="badge">修正方法</strong> alt がリンクの目的を示すようにしてください。</p>${why.imageLinks}`,
    ...
    SUS_ALT: `<p>この画像の alt には <i>%(alt)</i> という語が含まれています：</p><p><i>%(ALT_TEXT)</i></p><p><strong class="badge">修正方法</strong> 画像が何を意味するのかを簡潔に伝える alt にしてください。</p><div class="why"><p>ヒント：スクリーンリーダーは alt を読む前に「画像」と伝えるため、「〜の画像」「〜の写真」といった語は通常不要です</p></div>`,
    // REVIEW
