(function(global, factory) {
  ...
  const testNames = {
    ...
    TABLES_INVALID_HEADERS_REF: "この表には無効なヘッダー参照があります",
  ...
  const tips = {
    ALT_FILE_EXT: `<p><span style="display: none">%(alt)</span><strong>Alt テキスト：</strong> <i>%(ALT_TEXT)</i></p><p>スクリーンリーダーはこの URL を一文字ずつ読み上げることがあり、画像を見たときと同じ意味にはなりません。</p><p>${why.fix}この画像がこの文脈で何を意味するのか、簡潔に説明してください。</p>${why.images}`,
    ...
    ARIA_INPUT_FIELD_NAME: `<p><strong>要素：</strong> <code>%(EL)</code></p><p>${why.fix}有効なラベルを提供してください。カスタム入力要素には内部テキスト、title、aria-label、または aria-labelledby 属性がよく使われます。`,
    ...
    BTN_TIP: `${why.buttons}`,
    BTN_ROLE_IN_NAME: `<p><strong>スクリーンリーダー用ラベル：</strong> <i>%(TEXT)</i></p><p>スクリーンリーダーは「ボタン」という語を使ってボタンを説明していることを伝えるため、ラベル内の「button」は冗長になります。</p><p>${why.fix}ボタンのラベルは動作と一致させてください。可視ラベルがテキストではなくアイコンの場合は、アイコンの意味（例：「再生」「検索」「メニュー」）をラベルにしてください。</p>`,
    ...
    LABEL_IN_NAME: `<p><strong>可視テキスト：</strong> <i>%(TEXT)</i></p><p><strong>スクリーンリーダー用ラベル：</strong> <i>%(TEXT)</i></p><p>この要素の視覚的なテキストとアクセシブルネームが異なっている可能性があります。スクリーンリーダー利用者を混乱させたり、音声操作が正常に働かない可能性があります。</p><p>${why.fix}可視ラベルが非表示ラベルのテキストで始まり、非表示ラベルに含まれていない重要な情報が可視ラベルに含まれないようにしてください。</p>`,
    LINK_ALT_FILE_EXT: `<p><span style="display: none;">%(ALT)</span>Alt テキスト：<i>%(alt)</i></p><p>この alt テキストは、意味のあるリンクラベルではなく、ファイル名である可能性があります。</p><p>${why.fix}リンク画像の alt テキストには、リンク先のタイトルを使用してください。</p><div class="why"><p>alt テキストは画像が何を「意味するか」を伝えるためのものです。リンク画像の場合、その意味はリンク先です：</p><ul><li>"文字の書かれたページ" は画像の説明であり、リンクの説明ではありません。</li><li>"IMG_1234.jpg" はただのファイル名です。</li><li>"イベント申込書（.doc）" はリンク先の説明です。</li></ul></p></div>`,
    ...
    LINK_UNPRONOUNCEABLE: `<p>${why.fix}リンク先を説明するテキストまたはタイトルを追加するか、タイプミスまたはリンクされたスペース文字の場合は削除してください。</p><div class="why"><p>ヒント：スクリーンリーダーはスペースまたはシンボルのみを含むリンクを説明することはできません。沈黙したまま（「リンク、[...リンクタイトルがあるべき場所での不快な間...]」）、またはシンボルの名前を読み上げます。</p></div>`,
    ...
    LINK_STOPWORD: `<p>リンクテキスト：<i>%(text)</i></p><p><strong class="badge">修正方法</strong> リンク先を説明するテキストを使用してください。</p>${why.links}`,
    LINK_STOPWORD_ARIA: `<p>ARIA では有意義な名前が提供されていますが、可視テキストは一般的で分かりにくい状態です：<i>%(ERROR)</i></p><p>${why.fix}可視テキストも意味のある内容にし、アクセシブルネームと一致させてください。</p>${why.links}`,
    ...
    MISSING_ALT: `<p>画像に alt がない場合、スクリーンリーダーは画像ファイルの URL を 1 文字ずつ読み上げます。</p><p>${why.fix}この画像がこの文脈で何を意味するのか、簡潔に説明してください。</p>${why.images}`,
    ...
    QA_FAKE_HEADING: `<p><strong>太字テキスト：</strong> <i>%(text)</i></p><p>${why.fix}このテキストが話題の切り替えを示すために太字になっているのであれば、スクリーンリーダー利用者がページを移動する手がかりにできるよう、見出しとしてタグ付けしてください。</p><div class="why"> <p>ヒント：太字や斜体は視覚的な強調を与えますが、支援技術のための文書の目次には自動的には追加されません。</div>`,
    ...
    SUS_ALT: `<p>この画像の alt には「<i>%(alt)</i>」という語が含まれています。スクリーンリーダーは alt を読む前に「画像」と伝えるため、「〜の画像」「〜の写真」といった語は通常不要です。</p><p><strong>Alt テキスト：</strong> <i>%(ALT_TEXT)</i></p><p>${why.fix}この画像がこの文脈で何を意味するのか、簡潔に説明してください。</p><div class="why"><p>ヒント：形式そのものが意味を持つ場合、これらの語は冗長ではありません：<br>「壁に飾られた額入りの猫の写真。」</div>`,
    // REVIEW
    TABINDEX_ATTR: `<p>0 より大きい tabindex の値は、フォーカス可能な要素をその可視テキストとは異なる位置のフォーカス順に配置するため、支援技術利用者がそれらを見つけて操作することを困難にします。</p><p>${why.fix}代わりに HTML 内の要素の順序を変更して、フォーカス順と読み上げ順が一致するようにしてください。</p><div class="why"><p>ヒント：ページ上の単語の視覚的な順序と、キーボードのタブ順は通常一致しています。</p><p>正の tabindex を設定すると、タブ順だけが先頭に移動しますが、<strong>視覚順や読み上げ順は移動しません</strong>。</p></div>`,
