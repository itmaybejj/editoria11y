(function(global, factory) {
  ...
  const testNames = {
    ...
    TABLES_INVALID_HEADERS_REF: "A táblázatban érvénytelen fejléchivatkozások találhatók",
  ...
  const tips = {
    ALT_FILE_EXT: `<p><span style="display: none">%(alt)</span><strong>Alt szöveg:</strong> <i>%(ALT_TEXT)</i></p><p>A képernyőolvasók gyakran betűnként olvassák fel ezt az URL-t. Ez nem adja vissza a kép látványának jelentését.</p><p>${why.fix}Tömören írja le, mit jelent ez a kép ebben a kontextusban.</p>${why.images}`,
    ...
    ARIA_INPUT_FIELD_NAME: `<p><strong>Elem:</strong> <code>%(EL)</code></p><p>${why.fix}Adjon meg bármilyen érvényes címkét; az egyéni beviteli elemeknél ez általában belső szöveget, title, aria-label vagy aria-labelledby attribútumot jelent.`,
    ...
    LABEL_IN_NAME: `<p><strong>Látható szöveg:</strong> <i>%(TEXT)</i></p><p><strong>Képernyőolvasó‑címke:</strong> <i>%(TEXT)</i></p><p>Ennek az elemnek a látható szövege eltér a hozzáférhető névtől. Ez megzavarhatja a képernyőolvasó‑felhasználókat, és problémát okozhat a hangvezérlésben.</p><p>${why.fix}Győződjön meg róla, hogy a látható címke a rejtett címke szövegével kezdődik, és nem tartalmaz olyan jelentéssel bíró információt, amely a rejtett címkéből hiányzik.</p>`,
    LINK_ALT_FILE_EXT: `<p><span style="display: none;">%(ALT)</span>Alt szöveg: <i>%(alt)</i></p><p>Ez az alt szöveg valószínűleg egy fájlnév, nem pedig a link célját leíró címke.</p><p>${why.fix}Képlinkeknél használja a link céloldalának címét alt szövegként.</p><div class="why"><p>Az alt célja a kép jelentésének közvetítése. Képlink esetén ez maga a céloldal:</p><ul><li>"Szöveges oldal" — a képet írja le, nem a linket.</li><li>"IMG_1234.jpg" — csak fájlnév.</li><li>"Eseményregisztrációs űrlap (.doc)" — a link célját írja le.</li></ul></p></div>`,
    ...
    LINK_STOPWORD: `<p>Link szöveg: <i>%(text)</i></p><p><strong class="badge">Megoldás</strong> Használjon olyan linkszöveget, amely leírja a link célját.</p>${why.links}`,
    LINK_STOPWORD_ARIA: `<p>A link ARIA névvel rendelkezik, de a látható szöveg általános: <i>%(ERROR)</i>.</p><p>${why.fix}Legyen mindenki számára érthető, jelentéssel bíró linkszöveg, és egyezzen az ARIA névvel.</p>${why.links}`,
    ...
    MISSING_ALT: `<p>Alt hiányában a képernyőolvasók a kép URL‑jét olvassák fel, gyakran karakterenként.</p><p>${why.fix}Tömören írja le, mit jelent ez a kép ebben a kontextusban.</p>${why.images}`,
