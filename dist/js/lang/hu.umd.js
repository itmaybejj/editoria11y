(function(global, factory) {
  ...
  const tips = {
    ...
    BTN_TIP: `${why.buttons}`,
    BTN_ROLE_IN_NAME: `<p><strong>Hozzáférhető név:</strong> <i>%(TEXT)</i></p><p>A képernyőolvasók a „button” szóval jelzik, hogy gombot írnak le, ezért ez a szó felesleges.</p><p>${why.fix}A gomb címkéje egyezzen a műveletével. Ha a látható címke ikon a szöveg helyett, címkézze a gombot az ikon jelentésével, pl. „Lejátszás”, „Keresés” vagy „Menü”.</p>`,
    ...
    HIDDEN_FOCUSABLE: `<p><strong>Elem:</strong> <code>%(ELEMENT)</code></p><p>Ez az interaktív elem <code>aria-hidden="true"</code> beállítással rendelkezik, mégis fókuszálható. Ha el akarja rejteni a képernyőolvasók elől, adjon hozzá <code>tabindex="-1"</code>. Ha nem, távolítsa el az <code>aria-hidden</code> attribútumot.</p>`,
    ...
    LABEL_IN_NAME: `<p><strong>Látható szöveg:</strong> <i>%(TEXT)</i></p><p><strong>Hozzáférhető név:</strong> <i>%(TEXT)</i></p><p>Ennek az elemnek a látható szövege eltér a hozzáférhető névtől. Ez megzavarhatja a képernyőolvasó‑felhasználókat, és problémát okozhat a hangvezérlésben.</p><p>${why.fix}Győződjön meg róla, hogy a látható címke a rejtett címke szövegével kezdődik, és nem tartalmaz olyan jelentéssel bíró információt, amely a rejtett címkéből hiányzik.</p>`,
    ...
    LINK_IMAGE_ALT: `Ügyeljen arra, hogy az alt szöveg a link célját írja le:</p><p> {L} {ALT} <i>%(ALT_TEXT)</i></p>${why.imageLinks}`,
    ...
    QA_IN_PAGE_LINK: `<p><strong>ID:</strong> <code>%(ID)</code></p><p>A hivatkozás célja nem található a lapon.</p><div class="why"><p>Megjegyzés fejlesztőknek: ha ez JavaScript‑es horgony, ellenőrizze, hogy billentyűzettel is működik, mielőtt a kivételek közé veszi.</p></div>`,
