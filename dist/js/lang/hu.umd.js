(function(global, factory) {
  ...
  const testNames = {
    ...
    META_LANG_VALID: "Ennek az elemnek a nyelvkódja érvénytelen",
  ...
  const tips = {
    ALT_FILE_EXT: `<p><span style="display: none">%(alt)</span>Alt szöveg: <i>%(ALT_TEXT)</i></p><p>A képernyőolvasók gyakran betűnként olvassák fel ezt az URL-t. Ez nem adja vissza a kép látványának jelentését.</p><p><strong class="badge">Megoldás</strong> Ha a kép csupán dekoráció, adjon alt="" értéket, hogy a képernyőolvasók figyelmen kívül hagyják. Ha jelentése van, adjon meg leíró alt szöveget.</p>${why.images}`,
    ALT_MAYBE_BAD: `<p>Alt szöveg: <i>%(alt)</i></p><p><strong class="badge">Megoldás</strong> Állítsa be ezt a kép alt szövegét rövid, a kontextusnak megfelelő jelentésleírásra.</p>${why.images}`,
    ALT_MAYBE_BAD_WARNING: `<p>Alt szöveg: <i>%(alt)</i></p><p><strong class="badge">Megoldás</strong> Állítsa be ezt a kép alt szövegét rövid, a kontextusnak megfelelő jelentésleírásra.</p>${why.images}`,
    ALT_PLACEHOLDER: `<p>Alt szöveg: <i>%(alt)</i></p><p><strong class="badge">Megoldás</strong> Állítsa be ezt a kép alt szövegét rövid, a kontextusnak megfelelő jelentésleírásra.</p>${why.images}`,
    ALT_UNPRONOUNCEABLE: `<p>Alt szöveg: <i>%(alt)</i></p><p>Ez az alt szöveg csak kiejthetetlen karaktereket és/vagy szóközöket tartalmaz. A képernyőolvasók bejelentik, hogy kép található itt, majd zavartan szünetelnek vagy értelmezhetetlen hangzást olvasnak fel.</p><p><strong class="badge">Megoldás</strong> Adjon meg leíró alt szöveget, vagy ha ez csak ikon/spacer, használjon teljesen üres alt="" értéket.</p>${why.images}`,
    ...
    LINK_ALT_FILE_EXT: `<p><span style="display: none;">%(ALT)</span>Alt szöveg: <i>%(alt)</i></p><p>Ez az alt szöveg valószínűleg egy fájlnév, nem pedig a link célját leíró címke.</p><p><strong class="badge">Megoldás</strong> Állítsa be a kép alt szövegét a link céljának nevére.</p><div class="why"><p>Az alt célja a kép jelentésének közvetítése. Képlink esetén ez maga a céloldal:</p><ul><li>"Szöveges oldal" — a képet írja le, nem a linket.</li><li>"IMG_1234.jpg" — csak fájlnév.</li><li>"Eseményregisztrációs űrlap (.doc)" — a link célját írja le.</li></ul></p></div>`,
    LINK_ALT_MAYBE_BAD: `<p>Alt szöveg: <i>%(alt)</i></p><p><strong class="badge">Megoldás</strong> Állítsa a kép alt szövegét a link céljának címére.</p>${why.imageLinks}`,
    LINK_ALT_MAYBE_BAD_WARNING: `<p>Alt szöveg: <i>%(alt)</i></p><p><strong class="badge">Megoldás</strong> Állítsa a kép alt szövegét a link céljának címére.</p>${why.imageLinks}`,
    LINK_ALT_UNPRONOUNCEABLE: `<p>A képlink alt szövege csak kimondhatatlan karaktereket és/vagy szóközöket tartalmaz: <i>%(ALT_TEXT)</i>. A link nem írható le.</p><p>${why.fix}Adjon meg alt szöveget, amely a link célját vagy funkcióját írja le.</p>${why.imageLinks}`,
    ...
    LINK_IMAGE_ALT: `<p><strong>Alt szöveg:</strong> <i>%(ALT_TEXT)</i></p><p>${why.fix}Ügyeljen arra, hogy az alt szöveg a link célját írja le, ne csupán a kép vizuális tartalmát:</p>${why.imageLinks}`,
    ...
    LINK_PLACEHOLDER_ALT: `<p>A képlink alt szövege helykitöltő: <i>%(alt)</i>.</p><p>${why.fix}Állítsa be a link célját leíró alt szöveget.</p>${why.imageLinks}`,
    ...
    LINK_SUS_ALT: `<p>Ennek a képnek az alt szövegében szerepel <i>%(alt)</i>, ami általában arra utal, hogy nem a link célját írja le.</p><p>Alt szöveg: <i>%(ALT_TEXT)</i></p><p><strong class="badge">Megoldás</strong> Győződjön meg róla, hogy az alt szöveg a link célját vagy funkcióját írja le.</p>${why.imageLinks}`,
    ...
    SUS_ALT: `<p>Ennek a képnek az alt szövegében szerepel a <i>%(alt)</i> szó:</p><p><i>%(ALT_TEXT)</i></p><p><strong class="badge">Megoldás</strong> Fogalmazza újra az alt szöveget, hogy röviden közvetítse a kép jelentését.</p><div class="why"><p>Tipp: a képernyőolvasók már bejelentik, hogy egy képet írnak le, így az olyan kifejezések, mint „kép erről” vagy „fénykép erről”, gyakran feleslegesek</p></div>`,
