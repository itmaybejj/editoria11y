...
var pl = {
  ...
  strings: {
    ...
    OUTLINE: 'Konspekt',
    READABILITY_DESC:
      'Pokazuje wynik czytelności w zakładce <strong>Konspekt</strong>, aby pomóc ocenić trudność czytania.',
    TITLE: 'Tytuł',
    ALT: 'ALT',
    IMAGES: 'Obrazy',
    EDIT: 'Edytuj',
    ...
    DEVELOPER_DESC:
      'Sprawdza problemy, których naprawa może wymagać znajomości kodowania, takie jak atrybuty HTML, formularze i inne.',
    ...
    NOT_VISIBLE:
      'Element, który próbujesz wyświetlić, nie jest widoczny; może być ukryty lub znajdować się wewnątrz akordeonu lub karty.',
    MISSING_ROOT:
      'Sprawdzono dostępność całej strony, ponieważ obszar docelowy nie istnieje: <code>%(root)</code>',
    MISSING_READABILITY_ROOT:
      'Ocena czytelności opiera się na obszarze treści <code>%(fallback)</code>, ponieważ docelowy obszar <code>%(root)</code> nie istnieje.',
    SKIP_TO_PAGE_ISSUES: 'Przejdź do problemów ze stroną',
    CONSOLE_ERROR:
      'Przepraszamy, ale wystąpił problem z narzędziem do sprawdzania ułatwień dostępu na tej stronie. Czy możesz <a href="%(link)">zgłosić to za pomocą tego formularza</a> lub na <a href="%(link)">GitHub</a>?',
    APPEARANCE: 'Wygląd',
    ...
    DISMISSED: 'Odrzucone',
    DISMISS_REMINDER:
      'Proszę pamiętać, że ostrzeżenia są <strong>tymczasowo</strong> odrzucone. Wyczyść historię przeglądarki i pliki cookie, aby przywrócić wszystkie wcześniej odrzucone ostrzeżenia na wszystkich stronach.',
    ...
    COLOUR_FILTER_MESSAGE:
      'Sprawdź elementy, które są trudne do zauważenia lub odróżnienia od innych kolorów.',
    ...
    HEADING_SKIPPED_LEVEL:
      'Nagłówki nie powinny pomijać poziomów ani przeskakiwać z <strong>Nagłówka %(PREV_LEVEL)</strong> na <strong {C}>Nagłówek %(LEVEL)</strong>, ponieważ zakłóca to porządek i hierarchię treści, co utrudnia śledzenie. <hr> Jeśli <strong {C}>%(HEADING)</strong> znajduje się pod sekcją <strong>%(PREV_HEADING)</strong>, rozważ jego sformatowanie jako <strong>Nagłówek %(LEVEL)</strong>.',
    HEADING_EMPTY:
      'Znaleziono pusty nagłówek! Aby to naprawić, usuń tę linię lub zmień jej format z <strong {C}>Nagłówek %(level)</strong> na <strong>zwykły tekst</strong> lub <strong>akapit</strong>.',
    HEADING_LONG:
      'Nagłówek jest zbyt długi! Nagłówki służą do organizowania treści i przekazywania struktury. Powinny być krótkie, jasne, opisowe i niepowtarzalne. Pisz nagłówki nie dłuższe niż %(MAX_LENGTH) znaków (nie więcej niż zdanie). <hr> <strong>%(HEADING_LENGTH) Znaków</strong>',
    HEADING_FIRST:
      'Pierwszym nagłówkiem na stronie powinien być zwykle Nagłówek H1 lub Nagłówek H2. Nagłówek H1 jest głównym nagłówkiem opisującym ogólny cel strony i powinien być początkiem obszaru treści głównej. Dowiedz się więcej o <a href="https://www.w3.org/WAI/tutorials/page-structure/headings/">Strukturze nagłówków.</a>',
    HEADING_MISSING_ONE:
    ...
    LINK_EMPTY_LABELLEDBY:
      'Link ma wartość <code>aria-labelledby</code>, która jest pusta lub nie pasuje do wartości atrybutu <code>id</code> innego elementu na stronie.',
    LINK_EMPTY_NO_LABEL:
      'Łącze nie ma opisowego tekstu, który jest widoczny dla czytników ekranu i innych technologii wspomagających. Aby naprawić: <ul><li>Dodaj zwięzły tekst, który opisuje, dokąd prowadzi łącze.</li><li>Jeśli łączem jest <a href="https://a11y-101.com/development/icons-and-links">ikona lub SVG,</a> prawdopodobnie brakuje mu opisowej etykiety.</li><li>Jeśli uważasz, że to łącze jest błędem spowodowanym błędem kopiuj/wklej, rozważ usunięcie go.</li></ul>',
    LINK_STOPWORD:
      'Tekst linku może być niewystarczająco opisowy poza kontekstem: <strong {C}>%(ERROR)</strong>',
    LINK_STOPWORD_ARIA:
      'Mimo że podano dostępny tytuł, rozważ poprawę widocznego tekstu linku. Wyrażenia takie jak &quot;<strong {C}>%(ERROR)</strong>&quot; nie są znaczące.',
    LINK_TIP:
      '<hr> <strong>Wskazówka!</strong> Używaj jasnego i unikalnego tekstu linku, który opisuje cel linku, zwykle tytuł strony lub dokumentu.',
    LINK_CLICK_HERE:
      'Zwrot "kliknij" lub "kliknij tutaj" skupia się na mechanice myszy, gdy tymczasem wiele osób nie korzysta z myszy lub może przeglądać tę witrynę na urządzeniu mobilnym. Rozważ użycie innego czasownika związanego z zadaniem.',
    DUPLICATE_TITLE:
      'Atrybut <code>title</code> w linkach i obrazach ma na celu dostarczenie dodatkowych informacji i powinien być <strong>inny</strong> niż tekst lub tekst alternatywny. Tekst tytułu pojawia się po najechaniu kursorem na element, ale nie jest dostępny z poziomu klawiatury lub dotykowego wprowadzania. Rozważ <a href="https://www.a11yproject.com/posts/title-attributes/">całkowite unikanie atrybutu title.</a>',
    LINK_SYMBOLS:
      'Unikaj używania symboli jako wezwania do działania w tekście linków, chyba że są ukryte przed technologiami asystującymi. Czytniki ekranu mogą odczytywać symbole na głos, co może być mylące. Rozważ ich usunięcie: <strong {C}>%(ERROR)</strong>',
    LINK_URL:
      'Dłuższe, mniej zrozumiałe adresy URL używane jako tekst linku mogą być trudne do zrozumienia podczas korzystania z technologii wspomagającej. W&nbsp;większości przypadków zamiast adresu URL lepiej jest używać tekstu czytelnego dla człowieka. Krótkie adresy URL (takie jak głównej strony witryny) są w porządku.',
    LINK_DOI:
      'W przypadku stron internetowych lub zasobów dostępnych tylko online, przewodnik <a href="https://apastyle.apa.org/style-grammar-guidelines/paper-format/accessibility/urls#:~:text=descriptive%20links">APA Style</a> zaleca stosowanie linków opisowych poprzez zawijanie adresu URL lub DOI pracy wokół jej tytułu. Dłuższe, mniej zrozumiałe adresy URL używane jako tekst linku mogą być trudne do zrozumienia podczas korzystania z technologii wspomagającej.',
    LINK_NEW_TAB:
    ...
    // Images
    ALT_UNPRONOUNCEABLE:
      'Tekst alternatywny zawiera tylko niewymawialne symbole i/lub spacje. Czytniki ekranu ogłoszą obraz, a następnie zatrzymają się. Jeśli obraz jest dekoracyjny, upewnij się, że w tekście alternatywnym nie ma spacji. <hr> {ALT} <strong {C}>%(ALT_TEXT)</strong>',
    LINK_ALT_UNPRONOUNCEABLE:
      'Tekst alternatywny w tym połączonym obrazie zawiera tylko niewymawialne symbole i/lub spacje. Czytniki ekranu ogłoszą obraz, a następnie zatrzymają się. Upewnij się, że tekst alternatywny opisuje cel linku. <hr> {L} {ALT} <strong {C}>%(ALT_TEXT)</strong>',
    ...
    LINK_ALT_FILE_EXT:
      'Tekst alternatywny nie powinien zawierać rozszerzeń plików ani wymiarów obrazów. Upewnij się, że tekst alternatywny opisuje miejsce docelowe łącza, a nie treść lub wygląd obrazu. Usuń wyraz(y): <strong {C}>%(ERROR)</strong> <hr> {L} {ALT} <strong {C}>%(ALT_TEXT)</strong>',
    LINK_PLACEHOLDER_ALT:
      'Znaleziono nieopisowy lub zastępczy tekst alt w obrazie będącym łączem. Upewnij się, że tekst alternatywny opisuje miejsce docelowe łącza, a nie treść lub wygląd obrazu. Zastąp następujący tekst alt. <hr> {L} {ALT} <strong {C}>%(ALT_TEXT)</strong>.',
    LINK_SUS_ALT:
      'Technologie wspomagające już wskazują, że jest to obraz, więc &quot;<strong {C}>%(ERROR)</strong>&quot; mogą być zbędne. Upewnij się, że tekst alternatywny opisuje miejsce docelowe łącza, a nie treść lub wygląd obrazu. <hr> {L} {ALT} <strong {C}>%(ALT_TEXT)</strong>',
    ALT_FILE_EXT:
      'Tekst alternatywny nie powinien zawierać rozszerzeń plików ani wymiarów obrazów. Upewnij się, że tekst alternatywny opisuje miejsce docelowe łącza, a nie treść lub wygląd obrazu. Usuń wyraz(y): <strong {C}>%(ERROR)</strong> <hr> {ALT} <strong {C}>%(ALT_TEXT)</strong>',
    ALT_PLACEHOLDER:
      'Znaleziono nieopisowy lub zastępczy tekst alt. Zamień poniższy tekst alt na coś bardziej znaczącego. <hr> {ALT} <strong {C}>%(ALT_TEXT)</strong>.',
    SUS_ALT:
      'Technologie wspomagające już wskazują, że jest to obraz, więc &quot;<strong {C}>%(ERROR)</strong>&quot; mogą być zbędne. <hr> {ALT} <strong {C}>%(ALT_TEXT)</strong>',
    ...
    ALT_MAYBE_BAD:
      'Tekst alternatywny może nie dostarczać przydatnych informacji lub nie być opisowy. Popraw poniższy tekst alt: <hr> {ALT} <strong {C}>%(ALT_TEXT)</strong>',
    ...
    LABELS_ARIA_LABEL_INPUT:
      'Pole danych ma dostępną nazwę (etykietę), ale upewnij się, że etykieta jest również widoczna. <hr> <strong {B}>Dostępna nazwa</strong> <strong {C}>%(TEXT)</strong>',
    LABELS_NO_FOR_ATTRIBUTE:
      'Z tym polem input nie jest skojarzona żadna etykieta (label). Dodaj atrybut <code>for</code> do etykiety z wartością <code>id</code>  pasującą do <code>id</code> tego pola. <hr> <strong {B}>ID</strong> <strong {C}>#%(id)</strong>',
    ...
    // Embedded content
    EMBED_VIDEO:
      'Upewnij się, że <strong>wszystkie filmy mają napisy rozszerzone.</strong> Zapewnianie napisów rozszerzonych dla wszystkich treści audio i wideo jest obowiązkowym wymogiem poziomu A. Napisy mają na celu wspieranie osób Głuchych i słabosłyszących.',
    EMBED_AUDIO:
      'Upewnij się, że istnieje <strong>transkrypcja dla wszystkich nagrań dźwiękowych.</strong> Zapewnianie transkrypcji treści audio jest obowiązkowym wymogiem poziomu A. Transkrypcje mają na celu wspieranie osób Głuchych i słabosłyszących, ale każdy może z nich skorzystać. Rozważ umieszczenie transkrypcji poniżej lub w panelu akordeonowym.',
    EMBED_DATA_VIZ:
    ...
    EMBED_UNFOCUSABLE:
      '&lt;code&gt;&lt;iframe&gt;&lt;/code&gt; z elementami, które można wybrać, nie powinno mieć <code>tabindex="-1"</code>. Osadzona zawartość nie będzie dostępna za pomocą klawiatury.',
    ...
    QA_SUBSCRIPT:
      'Opcji formatowania indeksu dolnego i indeksu górnego należy używać wyłącznie do zmiany pozycji tekstu w celu dostosowania do konwencji lub standardów typograficznych. Powinna <strong>nie</strong> być używana wyłącznie do celów prezentacji lub wyglądu. Formatowanie całych zdań stwarza problemy z czytelnością. Odpowiednie przypadki użycia obejmują wyświetlanie wykładników, liczb porządkowych, takich jak 4<sup>th</sup> zamiast czwartej, oraz wzorów chemicznych (H<sub>2</sub>O).',
    QA_NESTED_COMPONENTS:
      'Unikaj zagnieżdżania interaktywnych komponentów układu, takich jak umieszczanie akordeonów w zakładkach lub zakładek w akordeonach. Może to skomplikować nawigację, zwiększyć obciążenie poznawcze i prowadzić do pomijania treści.',
    QA_JUSTIFY:
      'Unikaj stosowania wyjustowanego tekstu, który jest wyrównany zarówno do lewego, jak i prawego marginesu. Może to być trudne do odczytania dla niektórych osób ze względu na nierówne odstępy między słowami. Używaj tekstu wyrównanego do lewej strony dla lepszej czytelności.',
    QA_SMALL_TEXT:
    ...
    // Developer
    DUPLICATE_ID:
      'Znaleziono <strong>duplikat ID</strong>. Wiadomo, że błędy zduplikowanego ID powodują problemy dla technologii pomocniczych podczas próby interakcji z treścią. Usuń lub zmień następujący ID. <hr> <strong {B}>ID</strong> <strong {C}>#%(id)</strong>',
    UNCONTAINED_LI:
      'Wszystkie elementy listy <code>&lt;li&gt;</code> muszą być umieszczone wewnątrz elementów <code>&lt;ul&gt;</code> nieuporządkowanych lub <code>&lt;ol&gt;</code> uporządkowanych. Ta struktura pomaga czytnikom ekranu dokładnie ogłaszać listę i jej elementy.',
    TABINDEX_ATTR: 'Element nie powinien mieć atrybutu <code>tabindex</code> większego niż 0.',

    // Meta checks
    META_LANG:
      'Język strony nie zadeklarowany! <a href="https://www.w3.org/International/questions/qa-html-language-declarations">Zadeklaruj język w znaczniku HTML.</a>',
    META_TITLE:
      'Brak tytułu strony! Podaj <a href="https://developer.mozilla.org/en-US/docs/Web/HTML/Element/title">tytuł strony.</a>',
    META_SCALABLE:
      'Usuń parametr <code>user-scalable="no"</code> w <a href="https://developer.mozilla.org/en-US/docs/Web/HTML/Viewport_meta_tag">meta tagu widoku</a>, aby umożliwić powiększanie.',
    META_MAX:
    ...
    BTN_EMPTY_LABELLEDBY:
      'Przycisk ma wartość <code>aria-labelledby</code>, która jest pusta lub nie pasuje do wartości <code>id</code> innego elementu na stronie.',
    BTN: 'przycisk',
    BTN_TIP:
      'Dowiedz się, jak utworzyć <a href="https://www.sarasoueidan.com/blog/accessible-icon-buttons/">dostępny przycisk.</a>',
    BTN_ROLE_IN_NAME:
    ...
    // Contrast
    CONTRAST_NORMAL:
      'Tekst normalnej wielkości powinien mieć współczynnik kontrastu co najmniej %(RATIO).',
    CONTRAST_LARGE:
      'Tekst dużej wielkości powinien mieć współczynnik kontrastu co najmniej %(RATIO).',
    CONTRAST_ERROR:
      'Tekst nie ma wystarczającego kontrastu w stosunku do tła, co utrudnia jego odczytanie.',
    CONTRAST_WARNING:
      'Kontrast tego tekstu jest nieznany i wymaga ręcznego sprawdzenia. Upewnij się, że tekst i tło mają mocny kontrast.',
    CONTRAST_ERROR_GRAPHIC:
      'Grafika nie ma wystarczającego kontrastu w stosunku do tła, co utrudnia jej dostrzeganie.',
    CONTRAST_WARNING_GRAPHIC: 'Kontrast tej grafiki jest nieznany i wymaga ręcznego sprawdzenia.',
    CONTRAST_TIP_GRAPHIC:
      'Grafiki i elementy interfejsu użytkownika powinny mieć współczynnik kontrastu co najmniej 3:1.',
    CONTRAST_OPACITY: 'Zwiększ przezroczystość, aby poprawić widoczność.',
    CONTRAST_APCA:
      'To nie ma wystarczającego kontrastu dla żadnego rozmiaru tekstu. Rozważ użycie tej kombinacji koloru i rozmiaru tekstu?',
    ...
    CONTRAST_PLACEHOLDER:
      'Tekst zastępczy w tym polu wejściowym nie ma wystarczającego kontrastu w stosunku do tła, co utrudnia jego odczytanie.',
    CONTRAST_PLACEHOLDER_UNSUPPORTED:
      'Kontrast tego tekstu zastępczego jest nieznany i wymaga ręcznego sprawdzenia. Upewnij się, że tekst i tło mają silnie kontrastujące kolory.',
    CONTRAST_INPUT:
      'Tekst w tym polu wejściowym nie ma wystarczającego kontrastu w stosunku do tła, co utrudnia jego odczytanie.',
    CONTRAST: 'Kontrast',
    UNKNOWN: 'Nieznane',
    FG: 'Pierwszy plan',
    BG: 'Tło',
