...
function alphaBlend$1(rgbaFG = [0, 0, 0, 1], rgbBG = [0, 0, 0], round = true) {
    ...
    if (shadowDetection) {
      ...
      if (root instanceof ShadowRoot) {
        ...
        while (node && node !== root.host) {
          ...
          const bgColor2 = convertToRGBA$1(styles2.backgroundColor);
...
function checkContrast(results, option) {
  ...
  Elements.Found.Svg.forEach(($el) => {
    ...
    if ((shapes.length === 1 || allSameColour) && complex.length === 0) {
      ...
      const resolvedFill = fill === "currentColor" ? convertToRGBA$1(getComputedStyle(shapes[0]).color, opacity) : convertToRGBA$1(fill, opacity);
      const resolvedStroke = stroke === "currentColor" ? convertToRGBA$1(getComputedStyle(shapes[0]).color, opacity) : convertToRGBA$1(stroke, opacity);
...
const ed11yLang = {
  ...
  strings: {
    ...
    // 2.3.10 // @todo translate
    DISMISS_ALL: "Ignore similar alerts on this page",
    // 2.3.10
    dismissOkAllButton: "Similar alerts on this page are OK",
    // @todo translate
    dismissOnSite: "Mark OK on all pages",
    // @todo translate
  ...
  testNames: {
    ...
    LINK_IMAGE_ALT_AND_TEXT_TEST_NAME: "Does this alt text make sense inside this link?",
    // 2.3.10.
...
function initializeContrastTools(container, contrastDetails) {
  ...
  if (contrastTools) {
    ...
    const updatePreview = () => {
      ...
      const useAAA = Constants.Global.contrastAAA;
